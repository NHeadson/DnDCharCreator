...
export function useFirestore() {
  ...
  // Save a character to Firestore (with global count limit)
  const saveCharacter = async (characterData) => {
    try {
      ...
      console.log('spellcastingAbilityName value:', sanitizedData.spellcastingAbilityName, typeof sanitizedData.spellcastingAbilityName)

      // Enforce a global character count limit to avoid overloading the server
      const MAX_CHARACTERS = Number.parseInt(import.meta.env.VITE_MAX_CHARACTERS || '50')
      try {
        const existing = await getDocs(collection(db, 'characters'))
        const totalCount = existing.size
        if (Number.isFinite(MAX_CHARACTERS) && totalCount >= MAX_CHARACTERS) {
          const message = `Character limit reached (${MAX_CHARACTERS}). Please delete existing characters or log in as admin to manage.`
          console.warn(message)
          return { success: false, error: message }
        }
      } catch (countErr) {
        console.warn('Unable to verify character count; proceeding with save.', countErr)
      }
