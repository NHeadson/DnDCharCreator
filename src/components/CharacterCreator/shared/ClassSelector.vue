      ...
      <v-col cols="12" lg="5" md="6">
        <v-card variant="outlined">
          ...
          <v-card-text class="pt-2 pb-3">
            <v-select
              v-model="props.character.class"
              :density="$vuetify.display.smAndDown ? 'comfortable' : 'compact'"
              :error="!!props.characterData?.classError"
              :error-messages="props.characterData?.classError"
              item-title="name"
              item-value="index"
              :items="props.characterData?.classData || []"
              label="Choose Your Class"
              :loading="props.characterData?.isLoadingClasses"
              variant="outlined"
            >
              <template #prepend>
            ...
            <v-select
              v-if="props.character.class && availableSubclasses.length > 0 && shouldShowSubclassSelector"
              v-model="props.character.subclass"
              class="mt-3"
              :density="$vuetify.display.smAndDown ? 'comfortable' : 'compact'"
              item-title="name"
              item-value="index"
              :items="availableSubclasses"
              :label="`Choose Your ${selectedClassInfo?.subclassType || 'Subclass'}`"
              variant="outlined"
            >
              <template #prepend>
        ...
        <v-card v-else-if="selectedClassInfo" class="class-preview-card" variant="tonal">
          ...
          <v-card-text class="pt-3 pb-2">
            ...
            <div class="mb-3 ms-6">
              ...
              <div class="ms-3">
                <div style="display: flex; gap: 6px; flex-wrap: wrap">
                  <v-tooltip location="top" text="The type of hit dice used to determine hit points when leveling up">
                    <template #activator="{ props }">
                      <v-chip v-bind="props" color="green" size="small" variant="tonal">
                        ...
                        <span class="chip-value">: d{{ selectedClassInfo.hitDie ? selectedClassInfo.hitDie.replace('D', '') : '-' }}</span>
                      </v-chip>
                    </template>
                  </v-tooltip>
                  <v-tooltip v-if="selectedClassInfo.savingThrows && selectedClassInfo.savingThrows.length" location="top" text="Ability scores this class is proficient in for saving throws">
                    <template #activator="{ props }">
                  ...
                  <v-chip
                    v-if="
                      selectedClassInfo.armorTraining &&
                      (selectedClassInfo.armorTraining.light || selectedClassInfo.armorTraining.medium || selectedClassInfo.armorTraining.heavy || selectedClassInfo.armorTraining.shields)
                    "
                    color="blue"
                    size="small"
                    variant="tonal"
            ...
            <div v-if="selectedClassInfo.weaponProficiencies?.length" class="mb-3 ms-6">
              ...
              <div class="ms-3">
                <div style="display: flex; gap: 6px; flex-wrap: wrap">
                  <v-tooltip v-for="wp in selectedClassInfo.weaponProficiencies" :key="'wp-' + wp" location="top" text="A weapon your character is trained to use effectively">
                    <template #activator="{ props }">
            ...
            <div v-if="selectedClassInfo.armorProficiencies?.length" class="mb-3 ms-6">
              ...
              <div class="ms-3">
                <div style="display: flex; gap: 6px; flex-wrap: wrap">
                  <v-tooltip v-for="ap in selectedClassInfo.armorProficiencies" :key="'ap-' + ap" location="top" text="Armor your character can wear without penalty">
                    <template #activator="{ props }">
            ...
            <div v-if="selectedClassInfo.toolProficiencies?.length" class="mb-3 ms-6">
              ...
              <div class="ms-3">
                <div style="display: flex; gap: 6px; flex-wrap: wrap">
                  <v-tooltip v-for="tp in selectedClassInfo.toolProficiencies" :key="'tp-' + tp" location="top" text="Tools your character can use proficiently">
                    <template #activator="{ props }">
            ...
            <div v-if="selectedClassInfo?.skillProficiencies?.from?.length" class="mb-3 ms-6">
              <div class="d-flex align-center mb-1">
                ...
                <span class="text-caption text-grey-darken-1 ms-2">(Choice of {{ selectedClassInfo.skillProficiencies.count }})</span>
              </div>
              <div class="ms-3">
                <div style="display: flex; gap: 6px; flex-wrap: wrap">
                  <v-tooltip v-for="sk in selectedClassInfo.skillProficiencies.from" :key="'sk-' + sk" location="top" text="Skills your character can choose to be trained in">
                    <template #activator="{ props }">
            ...
            <div
              v-if="
                ...
                (selectedClassInfo.spellcasting.level === 1 || selectedClassInfo.spellcasting.spellSlots > 0 || selectedClassInfo.spellcasting.cantripsKnown > 0) &&
            ...
            >
              ...
              <div class="ms-3">
                <v-chip v-if="selectedClassInfo.spellcasting.ability" class="me-2 mb-1" color="deep-purple" size="x-small" variant="flat">
                  <v-icon class="me-1" size="x-small">mdi-star-four-points</v-icon>
                  Ability: {{ selectedClassInfo.spellcasting.ability }}
                </v-chip>
                <v-chip v-if="selectedClassInfo.spellcasting.cantripsKnown" class="me-2 mb-1" color="deep-purple" size="x-small" variant="flat">
                  <v-icon class="me-1" size="x-small">mdi-flash</v-icon>
                  Cantrips: {{ selectedClassInfo.spellcasting.cantripsKnown }}
                </v-chip>
                <v-chip v-if="selectedClassInfo.spellcasting.spellsKnown" class="me-2 mb-1" color="deep-purple" size="x-small" variant="flat">
                  <v-icon class="me-1" size="x-small">mdi-book-open-variant</v-icon>
                  Spells: {{ selectedClassInfo.spellcasting.spellsKnown }}
                </v-chip>
                <v-chip v-if="selectedClassInfo.spellcasting.spellSlots" class="me-2 mb-1" color="deep-purple" size="x-small" variant="flat">
                  <v-icon class="me-1" size="x-small">mdi-cube-outline</v-icon>
                  Spell Slots: {{ selectedClassInfo.spellcasting.spellSlots }}
                </v-chip>
                <div v-if="selectedClassInfo.spellcasting.info && selectedClassInfo.spellcasting.info.length" class="mt-2">
                  <v-expansion-panels class="spellcasting-info-panels" elevation="0" multiple>
                    <v-expansion-panel v-for="(info, idx) in selectedClassInfo.spellcasting.info" :key="'spellinfo-' + idx">
            ...
            <!-- Level 1 Features -->
            <div v-if="selectedClassInfo && selectedClassInfo.features && selectedClassInfo.features.length" class="mb-3 ms-6">
              <div class="d-flex align-center mb-1">
                ...
                <v-expansion-panels class="feature-panel" elevation="0" multiple>
                  <v-expansion-panel v-for="feature in selectedClassInfo.features" :key="'feature-' + feature.name">
                    ...
                    <v-expansion-panel-text>
                      ...
                      <div v-if="feature.feature_specific && (feature.feature_specific.enemy_type_options || feature.feature_specific.terrain_type_options)">
                        <div class="mt-2 mb-1">
                        ...
                        <div v-if="feature.feature_specific.enemy_type_options">
                          <div class="feature-choice-chips">
                            <v-chip v-for="type in feature.feature_specific.enemy_type_options.from.options" :key="'enemy-' + type" class="ms-3 me-1 mb-1" color="amber" size="small">
                              {{ type }}
                            </v-chip>
                          </div>
                        </div>
                        <div v-if="feature.feature_specific.terrain_type_options">
                          <div class="feature-choice-chips">
                            <v-chip v-for="type in feature.feature_specific.terrain_type_options.from.options" :key="'terrain-' + type" class="ms-3 me-1 mb-1" color="teal" size="small">
                              {{ type }}
                            </v-chip>
                          </div>
                        </div>
            ...
            <!-- Starting Equipment -->
            <div v-if="selectedClassInfo.startingEquipment && selectedClassInfo.startingEquipment.length" class="mb-3 ms-6">
              <div class="d-flex align-center mb-1">
              ...
              <div class="ms-3">
                <v-chip v-for="item in selectedClassInfo.startingEquipment" :key="'equip-' + (item.name || item)" class="me-1 mb-1" color="error" size="small" variant="tonal">
            ...
            <!-- Equipment Choices -->
            <div v-if="selectedClassInfo.startingEquipmentOptions && selectedClassInfo.startingEquipmentOptions.length" class="mb-3 ms-6">
              <div class="d-flex align-center mb-1">
              ...
              <div class="ms-3">
                <div v-for="opt in selectedClassInfo.startingEquipmentOptions" :key="'eqopt-' + (opt.desc || opt.choose || Math.random())" class="mb-2">
                  <template v-if="opt.from && Array.isArray(opt.from.options)">
                    <span v-for="(choice, choiceIdx) in opt.from.options" :key="'choice-' + getEquipmentChoiceLabel(choice)">
...
function getEquipmentChoiceLabel(choice) {
  ...
  if (choice && choice.option_type === 'choice' && choice.choice) {
    ...
    if (choice.choice.choose && choice.choice.from && choice.choice.from.equipment_category && choice.choice.from.equipment_category.name) {
...
const selectedClassInfo = computed(() => {
  ...
  if (props.character.classDetails && (props.character.classDetails.id === props.character.class || props.character.classDetails.index === props.character.class.toLowerCase())) {
  ...
  if (props.characterData?.classData) {
    ...
    return props.characterData.classData.find((classInfo) => classInfo.id === props.character.class || classInfo.index === props.character.class.toLowerCase())
  }
  return null
})
