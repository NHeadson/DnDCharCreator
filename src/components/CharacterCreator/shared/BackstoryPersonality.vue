<template>
  <v-card class="mb-6" variant="outlined">
    ...
    <v-card-text class="pt-3">
      ...
      <v-row class="mb-4">
        <v-col cols="12" md="6">
          <v-text-field v-model="character.name" density="compact" label="Character Name" placeholder="e.g., Figwit Goldbeard" variant="outlined">
            <template #prepend>
        ...
        <v-col cols="12" md="6">
          <v-select v-model="character.alignment" density="compact" :items="alignments" label="Alignment" variant="outlined">
            <template #prepend>
      ...
      <div class="mb-4">
        ...
        <div v-if="backgroundPersonalityOptions?.traits?.length" class="mb-2">
          <v-chip-group v-model="selectedBackgroundTraits" multiple>
            <v-chip v-for="trait in backgroundPersonalityOptions.traits" :key="trait" color="pink" filter size="small" variant="outlined">
              {{ trait }}
            </v-chip>
          </v-chip-group>
          <div class="text-caption text-grey mt-1">Select traits from your background, or add custom ones below</div>
        </div>
        <v-combobox v-model="character.personality.traits" chips clearable density="compact" label="Personality Traits" multiple placeholder="Add personality traits..." variant="outlined" />
      </div>
      ...
      <div class="mb-4">
        ...
        <div v-if="backgroundPersonalityOptions?.ideals?.length" class="mb-2">
          <v-chip-group v-model="selectedBackgroundIdeals" multiple>
            <v-chip v-for="ideal in backgroundPersonalityOptions.ideals" :key="ideal" color="blue" filter size="small" variant="outlined">
              {{ ideal }}
            </v-chip>
          </v-chip-group>
      ...
      <div class="mb-4">
        ...
        <div v-if="backgroundPersonalityOptions?.bonds?.length" class="mb-2">
          <v-chip-group v-model="selectedBackgroundBonds" multiple>
            <v-chip v-for="bond in backgroundPersonalityOptions.bonds" :key="bond" color="orange" filter size="small" variant="outlined">
              {{ bond }}
            </v-chip>
          </v-chip-group>
      ...
      <div class="mb-4">
        ...
        <div v-if="backgroundPersonalityOptions?.flaws?.length" class="mb-2">
          <v-chip-group v-model="selectedBackgroundFlaws" multiple>
            <v-chip v-for="flaw in backgroundPersonalityOptions.flaws" :key="flaw" color="warning" filter size="small" variant="outlined">
              {{ flaw }}
            </v-chip>
          </v-chip-group>
          <div class="text-caption text-grey mt-1">Select flaws from your background, or add custom ones below</div>
        </div>
        <v-combobox v-model="character.personality.flaws" chips clearable density="compact" label="Character Flaws" multiple placeholder="Add character weaknesses or vices..." variant="outlined" />
      </div>
