  ...
  <v-card class="mb-6" variant="outlined">
    ...
    <v-card-text class="pt-3">
      <div v-if="character.feats.length === 0" class="text-center py-4">
        ...
        <div class="text-caption text-grey mt-1">Feats are typically gained at certain levels or through variant human rules</div>
      </div>
          ...
          <v-card class="feat-card" color="teal-lighten-5" flat>
            <v-card-text class="pa-3">
              <div class="d-flex justify-between align-center mb-2">
                ...
                <v-btn color="teal" icon size="x-small" variant="text" @click="removeFeat(index)">
                  <v-icon size="small">mdi-close</v-icon>
                </v-btn>
              </div>

              <v-text-field v-model="feat.name" class="mb-2" density="compact" label="Feat Name" variant="outlined" />

              <v-textarea v-model="feat.description" class="mb-2" density="compact" label="Feat Description" rows="3" variant="outlined" />

              <v-text-field v-model="feat.source" density="compact" label="Source (e.g., PHB, XGE)" variant="outlined" />
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>

      <v-btn class="mt-2" color="teal" prepend-icon="mdi-plus" variant="outlined" @click="addFeat">Add Feat</v-btn>
      ...
      <v-expansion-panels class="mt-4" variant="accordion">
        <v-expansion-panel>
          ...
          <v-expansion-panel-text>
            <v-row>
              <v-col v-for="feat in popularFeats" :key="feat.name" cols="12" md="4" sm="6">
...
const popularFeats = [
  ...
  },
  {
    ...
    description:
      "You've learned to put the weight of a weapon to your advantage. You can choose to take a -5 penalty to your attack roll. If you do so and the attack hits, you add +10 to the attack's damage.",
  },
  {
    name: 'Sharpshooter',
    shortDesc: 'Ranged weapon expertise',
    description:
      "You have mastered ranged weapons and can make shots that others find impossible. You can choose to take a -5 penalty to your attack roll. If you do so and the attack hits, you add +10 to the attack's damage.",
  },
  {
    name: 'War Caster',
  ...
  },
  {
    ...
    description:
      'You have inexplicable luck that seems to kick in at just the right moment. You have 3 luck points. Whenever you make an attack roll, ability check, or saving throw, you can spend one luck point to roll an additional d20.',
  },
  {
    name: 'Tough',
  ...
  },
  {
    ...
    description:
      'You learn the misty step spell and one 1st-level spell of your choice from the divination or enchantment school of magic. You can cast each of these spells without expending a spell slot once per long rest.',
  },
  {
    name: 'Telekinetic',
