  ...
  <v-card flat>
    ...
    <div class="hero-section text-center mb-8">
      <div class="character-avatar-section mb-4">
        <v-avatar class="elevation-4" size="120">
          <v-img v-if="character.portrait" :src="character.portrait" cover>
            <template #placeholder>
              <v-icon color="primary" size="60">mdi-account</v-icon>
            </template>
          </v-img>
          <v-icon v-else color="primary" size="60">mdi-account</v-icon>
        </v-avatar>
      </div>

      <div class="character-title-section">
        <h1 class="text-h3 font-weight-bold mb-2">{{ character.name || 'Unnamed Character' }}</h1>
        <div class="character-subtitle text-h6 text-primary mb-3">Level {{ character.level }}</div>
      ...
      <v-row class="ma-0">
        ...
        <v-col class="order-3 order-lg-1" cols="12" lg="4" md="4">
          ...
          <v-card class="section-card mb-4" variant="outlined">
            ...
            <v-card-text class="pa-3">
              ...
              <div v-if="character.classDetails?.savingThrows?.length" class="mb-3">
                ...
                <v-chip v-for="save in character.classDetails.savingThrows" :key="save" class="ma-1" color="success" size="default" variant="tonal">
                  {{ save }}
                </v-chip>
              </div>
              ...
              <div v-if="character.classDetails?.armorTraining" class="mb-3">
                ...
                <v-chip v-if="character.classDetails.armorTraining.light" class="ma-1" color="info" size="default" variant="tonal">Light Armor</v-chip>
                <v-chip v-if="character.classDetails.armorTraining.medium" class="ma-1" color="info" size="default" variant="tonal">Medium Armor</v-chip>
                <v-chip v-if="character.classDetails.armorTraining.heavy" class="ma-1" color="info" size="default" variant="tonal">Heavy Armor</v-chip>
                <v-chip v-if="character.classDetails.armorTraining.shields" class="ma-1" color="info" size="default" variant="tonal">Shields</v-chip>
              </div>

              <!-- Languages -->
              <div v-if="allLanguages.length">
                <div class="subsection-title mb-2">🗣️ Languages</div>
                <v-chip v-for="lang in allLanguages" :key="lang" class="ma-1" color="secondary" size="default" variant="tonal">
                  {{ lang }}
                </v-chip>
              </div>
            </v-card-text>
          ...
          <v-card v-if="character.classDetails?.features?.length" class="section-card mb-4" variant="outlined">
            ...
            <v-card-text class="pa-0">
              <v-expansion-panels class="trait-panels" variant="accordion">
                <v-expansion-panel v-for="feature in character.classDetails.features" :key="feature.name" class="trait-panel">
                  <v-expansion-panel-title class="pa-3">
                    <span class="font-weight-medium">{{ feature.name }}</span>
                  </v-expansion-panel-title>
                  <v-expansion-panel-text class="trait-description pa-3">
                    <div class="text-body-2">{{ feature.desc || feature.description || 'No description available.' }}</div>
                  </v-expansion-panel-text>
                </v-expansion-panel>
              </v-expansion-panels>
            </v-card-text>
              ...
              <v-row dense>
                <v-col v-for="(score, ability) in character.abilityScores" :key="ability" cols="6">
                  <v-card class="ability-score-card text-center pa-3" variant="outlined">
                    ...
                    <div class="ability-modifier text-h5 font-weight-bold my-1">{{ score.modifier >= 0 ? '+' : '' }}{{ score.modifier }}</div>
        ...
        <v-col class="order-2 order-lg-3" cols="12" lg="4" md="4">
            ...
            <v-card-text class="pa-3">
              <div v-if="character.personality?.traits?.length" class="personality-section mb-4">
                ...
                <div class="personality-items">
                  <v-chip v-for="trait in character.personality.traits" :key="trait" class="personality-chip ma-1" color="pink" size="small" variant="tonal">
                    {{ trait }}
                  </v-chip>
                </div>
              </div>
              ...
              <div v-if="character.personality?.ideals?.length" class="personality-section mb-4">
                ...
                <div class="personality-items">
                  <v-chip v-for="ideal in character.personality.ideals" :key="ideal" class="personality-chip ma-1" color="blue" size="small" variant="tonal">
                    {{ ideal }}
                  </v-chip>
                </div>
              </div>
              ...
              <div v-if="character.personality?.bonds?.length" class="personality-section mb-4">
                ...
                <div class="personality-items">
                  <v-chip v-for="bond in character.personality.bonds" :key="bond" class="personality-chip ma-1" color="orange" size="small" variant="tonal">
                    {{ bond }}
                  </v-chip>
                </div>
              </div>
              ...
              <div v-if="character.personality?.flaws?.length" class="personality-section">
                ...
                <div class="personality-items">
                  <v-chip v-for="flaw in character.personality.flaws" :key="flaw" class="personality-chip ma-1" color="warning" size="small" variant="tonal">
                    {{ flaw }}
                  </v-chip>
                </div>
              </div>
            ...
            <v-card-text class="pa-3">
              ...
              <div v-if="hasInventoryItems" class="mb-4">
                ...
                <div v-else class="d-flex flex-wrap ga-2">
                  <div v-for="(item, index) in allInventoryItems" :key="`${item?.name || 'unknown'}-${item?.source || 'unknown'}-${index}`" class="inventory-equipment-item">
                      ...
                      <div class="equipment-pack-container mb-3" style="width: 100%">
                        ...
                        <div v-if="getEquipmentTooltip(item.name)?.contents" class="pack-contents mt-2 ms-2">
                          ...
                          <div class="d-flex flex-wrap ga-2">
                            <v-tooltip v-for="contentItem in getEquipmentTooltip(item.name).contents" :key="contentItem" location="top" max-width="300">
                              <template #activator="{ props: tooltipProps }">
                                <v-chip v-bind="tooltipProps" class="pack-content-chip" color="grey-darken-1" size="small" variant="outlined">
                              ...
                              <div v-if="getPackContentTooltip(contentItem)">
                                <div v-if="getPackContentTooltip(contentItem).type" class="text-caption mb-1">
                                  <strong>Type:</strong>
                                  {{ getPackContentTooltip(contentItem).type }}
                                </div>
                              ...
                              <div v-else>
                                <div class="text-caption">{{ capitalizeEquipmentName(contentItem) }} - Part of {{ capitalizeEquipmentName(item.name) }}</div>
                              </div>
                            </v-tooltip>
                          </div>
                        </div>
                    ...
                    <template v-else-if="item && item.name && isSpecificMusicalInstrument(item)">
                      ...
                      <v-tooltip v-else location="top" :text="`Musical instrument from your ${item.source === 'class' ? 'class equipment' : 'background'}`">
                        <template #activator="{ props: tooltipProps }">
                          <v-chip
                            v-bind="tooltipProps"
                            class="ma-1"
                    ...
                    <template v-else-if="item && item.name">
                      <v-tooltip v-if="getEquipmentTooltip(item.name)" location="top" max-width="300">
                        ...
                        <template #default>
                          ...
                          <div v-if="getEquipmentTooltip(item.name).properties && getEquipmentTooltip(item.name).properties !== 'None'" class="text-caption mb-1">
              ...
              <div v-if="hasCoins" class="coins-section">
                ...
                <div class="coins-display d-flex flex-wrap">
                  <v-chip v-if="character.coins.pp" class="ma-1" color="purple" size="small" variant="tonal">{{ character.coins.pp }} Platinum</v-chip>
                  <v-chip v-if="character.coins.gp" class="ma-1" color="yellow-darken-2" size="small" variant="outlined">{{ character.coins.gp }} Gold</v-chip>
                  <v-chip v-if="character.coins.ep" class="ma-1" color="blue-grey" size="small" variant="outlined">{{ character.coins.ep }} Electrum</v-chip>
                  <v-chip v-if="character.coins.sp" class="ma-1" color="grey" size="small" variant="outlined">{{ character.coins.sp }} Silver</v-chip>
                  <v-chip v-if="character.coins.cp" class="ma-1" color="brown" size="small" variant="outlined">{{ character.coins.cp }} Copper</v-chip>
                </div>
              </div>
            </v-card-text>
          </v-card>
          ...
          <v-card v-if="character.speciesDetails?.darkvision" class="section-card mb-4" variant="outlined">
            ...
            <v-card-text class="px-3 pt-0">
              <v-chip class="mx-1 my-0 pl-3" color="indigo" prepend-icon="mdi-weather-night" size="small" variant="tonal">Darkvision {{ character.speciesDetails.darkvision }}ft</v-chip>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
...
const hasPersonality = computed(() => {
  ...
  return p && ((p.traits && p.traits.length) || (p.ideals && p.ideals.length) || (p.bonds && p.bonds.length) || (p.flaws && p.flaws.length))
})
...
const equipmentSummary = computed(() => {
  ...
  equipment
    .filter((item) => item.source === 'class-default' || !item.source)
    .forEach((item) => {
      const name = item.name || item
      if (!seenItems.has(name.toLowerCase())) {
        deduplicatedEquipment.push(item)
  ...
  equipment
    .filter((item) => item.source === 'background')
    .forEach((item) => {
      const name = item.name || item
      if (!seenItems.has(name.toLowerCase())) {
        deduplicatedEquipment.push(item)
...
const isSpecificMusicalInstrument = (item) => {
  ...
  return (
    (name.includes('lute') ||
      ...
      name.includes('clarinet')) &&
    !name.includes('musical instrument')
  ) // Don't match generic "musical instrument"
}
...
const isSpecificArtisanTool = (item) => {
  ...
  const name = item.name.toLowerCase()
  return (
    (name.includes('smith') && name.includes('tools')) ||
...
const isEquipmentPack = (item) => {
  ...
  return (
    name.includes('pack') &&
    (name.includes('entertainer') ||
      name.includes('diplomat') ||
      name.includes('dungeoneer') ||
...
const getEquipmentTooltip = (itemName) => {
  ...
  const tooltips = {
    ...
    "burglar's pack": {
      type: 'Equipment Pack',
      description:
        'Includes a backpack, a bag of 1,000 ball bearings, 10 feet of string, a bell, 5 candles, a crowbar, a hammer, 10 pitons, a hooded lantern, 2 flasks of oil, 5 days rations, a tinderbox, a waterskin, and 50 feet of hempen rope.',
      contents: [
        'Backpack',
        '1,000 Ball bearings',
        '10 ft String',
    ...
    "carpenter's tools": {
      ...
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft objects with wood.',
    },
    "potter's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft objects with clay.',
    },
    "smith's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft metal objects.',
    },
    "brewer's supplies": {
      type: "Artisan's Tools",
      description:
        'These special supplies include the items needed to pursue a craft or trade. Proficiency with these supplies lets you add your proficiency bonus to any ability checks you make to brew beer, ale, and other alcoholic beverages.',
    },
    "calligrapher's supplies": {
      type: "Artisan's Tools",
      description:
        'These special supplies include the items needed to pursue a craft or trade. Proficiency with these supplies lets you add your proficiency bonus to any ability checks you make to create beautiful writing.',
    },
    "cartographer's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to create maps.',
    },
    "cobbler's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft and repair shoes.',
    },
    "cook's utensils": {
      type: "Artisan's Tools",
      description:
        'These special utensils include the items needed to pursue a craft or trade. Proficiency with these utensils lets you add your proficiency bonus to any ability checks you make to prepare meals.',
    },
    "glassblower's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft glass objects.',
    },
    "jeweler's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft jewelry and appraise gems.',
    },
    "leatherworker's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft leather goods.',
    },
    "mason's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft stone structures.',
    },
    "painter's supplies": {
      type: "Artisan's Tools",
      description:
        'These special supplies include the items needed to pursue a craft or trade. Proficiency with these supplies lets you add your proficiency bonus to any ability checks you make to create paintings.',
    },
    "tinker's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft and repair mechanical devices.',
    },
    "weaver's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft cloth and textiles.',
    },
    "woodcarver's tools": {
      type: "Artisan's Tools",
      description:
        'These special tools include the items needed to pursue a craft or trade. Proficiency with these tools lets you add your proficiency bonus to any ability checks you make to craft wooden objects.',
    },
    "thieves' tools": {
      type: 'Tools',
    ...
    bagpipes: {
      ...
      description:
        'If you have proficiency with bagpipes, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The haunting sound of bagpipes can inspire allies or intimidate enemies.',
    },
    lute: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a lute, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The melodic strings of a lute are perfect for entertainment and bardic magic.',
    },
    flute: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a flute, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The clear, sweet notes of a flute can carry far and wide.',
    },
    drum: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with drums, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The rhythmic beat of drums can rally troops and set the pace for marching.',
    },
    dulcimer: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a dulcimer, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The gentle plucked strings create soothing melodies.',
    },
    horn: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a horn, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The bold call of a horn can signal across great distances.',
    },
    lyre: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a lyre, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The classical strings of a lyre evoke ancient traditions.',
    },
    'pan flute': {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a pan flute, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The breathy notes create pastoral melodies.',
    },
    shawm: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a shawm, you can add your proficiency bonus to any ability checks you make to play music with the instrument. This double-reed instrument produces loud, penetrating tones.',
    },
    viol: {
      type: 'Musical Instrument',
      description:
        'If you have proficiency with a viol, you can add your proficiency bonus to any ability checks you make to play music with the instrument. The bowed strings create rich, expressive tones.',
    },
...
const getPackContentTooltip = (itemName) => {
  ...
  const packContentTooltips = {
    ...
    'oil flask': {
      ...
      description: 'Oil usually comes in a clay flask that holds 1 pint. As an action, you can splash the oil in this flask onto a creature within 5 feet of you or throw it up to 20 feet.',
    },
...
const allInventoryItems = computed(() => {
  ...
  const hasClothes = allItemsAdded.some((name) => name.includes('clothes') || name.includes('costume') || name.includes('vestments'))
  if (!hasClothes) {
    inventory.push({ name: 'Common Clothes', source: 'basic', quantity: 1, category: 'gear' })
  } else {
    console.log(
      'Skipping Common Clothes - already have clothing:',
      allItemsAdded.filter((name) => name.includes('clothes') || name.includes('costume') || name.includes('vestments'))
    )
  }

  // Only add Pouch if there's no other pouch (like Belt Pouch)
  const hasPouch = allItemsAdded.some((name) => name.includes('pouch') || name.includes('bag') || name.includes('sack'))
  if (!hasPouch) {
    inventory.push({ name: 'Pouch', source: 'basic', quantity: 1, category: 'gear' })
  } else {
    console.log(
      'Skipping Pouch - already have container:',
      allItemsAdded.filter((name) => name.includes('pouch') || name.includes('bag') || name.includes('sack'))
    )
  }
...
const enhancedSpeciesTraits = computed(() => {
  ...
  return allTraits.map((trait) => {
    ...
    if (trait.description && trait.description.trim() && trait.description !== 'Description not loaded yet.' && !trait.description.includes('not loaded')) {
...
'Draconic Ancestry': 'You have draconic ancestry. Choose one type of dragon from the Draconic Ancestry table. Your breath weapon and damage resistance are determined by the dragon type.', 'Breath
Weapon': 'You can use your action to exhale destructive energy. Your draconic ancestry determines the size, shape, and damage type of the exhalation.', 'Damage Resistance': 'You have resistance to the
damage type associated with your draconic ancestry.', 'Darkvision': 'You have superior vision in dark and dim conditions. You can see in dim light within 60 feet of you as if it were bright light, and
in darkness as if it were dim light. You cannot discern color in darkness, only shades of gray.', 'Fey Ancestry': 'You have advantage on saving throws against being charmed, and magic cannot put you
to sleep.', 'Trance': 'Elves do not need to sleep. Instead, they meditate deeply, remaining semiconscious, for 4 hours a day. After resting in this way, you gain the same benefit that a human does
from 8 hours of sleep.', 'Keen Senses': 'You have proficiency in the Perception skill.', 'Dwarven Resilience': 'You have advantage on saving throws against poison, and you have resistance against
poison damage.', 'Stout Resilience': 'You have advantage on saving throws against poison, and you have resistance against poison damage.', 'Stonecunning': 'Whenever you make an Intelligence (History)
check related to the origin of stonework, you are considered proficient in the History skill and add double your proficiency bonus to the check.', 'Lucky': 'When you roll a 1 on the d20 for an attack
roll, ability check, or saving throw, you can reroll the die and must use the new roll.', 'Brave': 'You have advantage on saving throws against being frightened.', 'Halfling Nimbleness': 'You can move
through the space of any creature that is of a size larger than yours.', 'Natural Stealthiness': 'You can attempt to hide even when you are obscured only by a creature that is at least one size larger
than you.', 'Draconic Ancestry': 'You have draconic ancestry. Choose one type of dragon from the Draconic Ancestry table. Your breath weapon and damage resistance are determined by the dragon type.',
'Breath Weapon': 'You can use your action to exhale destructive energy. Your draconic ancestry determines the size, shape, and damage type of the exhalation.' } return allTraits.map(trait => {
console.log('Processing trait:', trait.name, 'with index:', trait.index) console.log(' - trait.description:', typeof trait.description, '|', trait.description) console.log(' - trait.desc:', typeof
trait.desc, '|', trait.desc) // Priority 1: Check if trait already has description in various possible formats if (trait.description && trait.description.trim()) { console.log('Using existing
description for:', trait.name, '|', trait.description) return { ...trait, desc: Array.isArray(trait.description) ? trait.description : [trait.description], description:
Array.isArray(trait.description) ? trait.description.join('\n') : trait.description, } } if (trait.desc && Array.isArray(trait.desc) && trait.desc.length && trait.desc[0].trim()) { console.log('Using
existing desc for:', trait.name, '|', trait.desc) return { ...trait, desc: trait.desc, description: trait.desc.join('\n'), } } // Priority 2: Try API data if available const traitDetail =
traitDetails.value[trait.index] if (traitDetail && traitDetail.desc) { console.log('Using API description for:', trait.name) return { ...trait, desc: traitDetail.desc, description:
Array.isArray(traitDetail.desc) ? traitDetail.desc.join('\n') : traitDetail.desc, } } // Priority 3: Use fallback descriptions const fallbackDesc = fallbackDescriptions[trait.name] if (fallbackDesc) {
console.log('Using fallback description for:', trait.name) return { ...trait, desc: [fallbackDesc], description: fallbackDesc, } } // Priority 4: Last resort - provide a basic description
console.log('Using default description for:', trait.name) return { // Priority 4: Last resort - provide a basic description console.log('Using default description for:', trait.name) return { ...trait,
desc: [`A species trait: ${trait.name}`], description: `A species trait: ${trait.name}`, } }) }) // Fallback trait descriptions for common traits (by name) const fallbackDescriptions = { 'Draconic
Ancestry': ['You have draconic ancestry. Choose one type of dragon from the Draconic Ancestry table. Your breath weapon and damage resistance are determined by the dragon type.'], 'Breath Weapon':
['You can use your action to exhale destructive energy. Your draconic ancestry determines the size, shape, and damage type of the exhalation.'], 'Damage Resistance': ['You have resistance to the
damage type associated with your draconic ancestry.'], 'Darkvision': ['You have superior vision in dark and dim conditions. You can see in dim light within 60 feet of you as if it were bright light,
and in darkness as if it were dim light. You cannot discern color in darkness, only shades of gray.'], 'Fey Ancestry': ['You have advantage on saving throws against being charmed, and magic cannot put
you to sleep.'], 'Trance': ['Elves do not need to sleep. Instead, they meditate deeply, remaining semiconscious, for 4 hours a day. After resting in this way, you gain the same benefit that a human
does from 8 hours of sleep.'], 'Keen Senses': ['You have proficiency in the Perception skill.'], 'Dwarven Resilience': ['You have advantage on saving throws against poison, and you have resistance
against poison damage.'], 'Stout Resilience': ['You have advantage on saving throws against poison, and you have resistance against poison damage.'], 'Stonecunning': ['Whenever you make an
Intelligence (History) check related to the origin of stonework, you are considered proficient in the History skill and add double your proficiency bonus to the check.'], 'Lucky': ['When you roll a 1
on the d20 for an attack roll, ability check, or saving throw, you can reroll the die and must use the new roll.'], 'Brave': ['You have advantage on saving throws against being frightened.'],
'Halfling Nimbleness': ['You can move through the space of any creature that is of a size larger than yours.'], 'Hellish Resistance': ['You have resistance to fire damage.'], 'Infernal Legacy': ['You
know the thaumaturgy cantrip. When you reach 3rd level, you can cast the hellish rebuke spell as a 2nd-level spell once with this trait and regain the ability to do so when you finish a long rest.'],
'Extra Language': ['You can speak, read, and write one extra language of your choice.'], 'Skill Versatility': ['You gain proficiency in two skills of your choice.'], 'Natural Armor': ['You have tough,
scaly skin. When you aren\'t wearing armor, your AC is 13 + your Dex modifier.'], }
<style scoped>
/* Hero Section */
.hero-section {
...
.ability-score-card:hover {
  ...
  background: linear-gradient(135deg, rgba(var(--v-theme-primary), 0.2), rgba(var(--v-theme-secondary), 0.12)) !important;
