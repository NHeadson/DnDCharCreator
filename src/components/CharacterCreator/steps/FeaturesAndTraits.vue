<template>
  <v-card flat>
    ...
    <v-card-subtitle class="text-center mb-4">Configure your character's abilities, proficiencies, and special features</v-card-subtitle>
    <v-card-text>
      <!-- Core Combat Stats -->
      <v-row class="mb-6">
        <v-col cols="12" md="4">
          <v-card class="combat-stat-card" color="error-lighten-4" variant="outlined">
            ...
            <v-card-text class="combat-card-content">
              <div class="text-center">
                ...
                <div class="text-body-2">
                  <strong>Base:</strong>
                  {{ getClassHitDie }} + {{ getConModifier }} (Con)
                </div>
              </div>
            </v-card-text>
          </v-card>
        ...
        <v-col cols="12" md="4">
          <v-card class="combat-stat-card" color="blue-lighten-4" variant="outlined">
        ...
        <v-col cols="12" md="4">
          <v-card class="combat-stat-card" color="green-lighten-4" variant="outlined">
            ...
            <v-card-text class="combat-card-content">
              <v-list class="pt-4 pb-0" density="compact">
                <v-list-item>
                  ...
                  <template #append>
                    <v-tooltip location="top" text="Added to attack rolls, skill checks, and saving throws you're proficient in">
                ...
                <v-list-item>
                  ...
                  <template #append>
                    <v-tooltip location="top" text="Your Dexterity modifier - determines turn order in combat">
                      <template #activator="{ props }">
                        <v-chip v-bind="props" :color="getDexModifier >= 0 ? 'success' : 'error'" size="small">{{ getDexModifier >= 0 ? '+' : '' }}{{ getDexModifier }}</v-chip>
                      </template>
                    </v-tooltip>
                  </template>
                </v-list-item>
        ...
        <v-card-text>
            ...
            <v-row>
              ...
              <v-col cols="12" md="8">
                <div class="d-flex flex-wrap align-center ga-2 mb-3">
                  <v-tooltip location="top" text="Physical size category - affects space occupied and some mechanics">
                    <template #activator="{ props }">
                      <v-chip v-bind="props" color="primary" prepend-icon="mdi-resize" size="small">{{ character.speciesDetails.size || 'Medium' }} Size</v-chip>
                    </template>
                  </v-tooltip>
                  <v-tooltip location="top" text="Base walking speed in feet per round during combat">
                    <template #activator="{ props }">
                      <v-chip v-bind="props" color="secondary" prepend-icon="mdi-run-fast" size="small">{{ character.speciesDetails.speed || 30 }} ft Speed</v-chip>
                    </template>
                  </v-tooltip>
                  <v-tooltip v-if="character.speciesDetails.darkvision" location="top" text="Can see in darkness as if it were dim light within this range">
                    <template #activator="{ props }">
                      <v-chip v-bind="props" color="purple" prepend-icon="mdi-eye" size="small">{{ character.speciesDetails.darkvision }} ft Darkvision</v-chip>
                    </template>
                  </v-tooltip>
                  <v-tooltip v-if="character.speciesDetails.damageResistance" location="top" text="Takes half damage from these damage types">
                    <template #activator="{ props }">
                      <v-chip v-bind="props" color="orange" prepend-icon="mdi-shield" size="small">
              ...
              <v-col cols="12" md="4">
                <div v-if="character.speciesDetails.ability_bonuses?.length" class="d-flex flex-wrap align-center ga-1">
                  ...
                  <v-tooltip
                    v-for="bonus in character.speciesDetails.ability_bonuses"
                    :key="bonus.ability_score?.name"
                    location="top"
                    :text="`Adds +${bonus.bonus} to your ${bonus.ability_score?.name} score`"
                  >
                    <template #activator="{ props }">
                      <v-chip v-bind="props" color="success" size="x-small">{{ bonus.ability_score?.name }}: +{{ bonus.bonus }}</v-chip>
                    </template>
                  </v-tooltip>
                </div>
              </v-col>
          ...
          <div v-if="character.speciesDetails.languages?.length" class="mb-4">
            <div class="d-flex flex-wrap align-center ga-2">
              ...
              <v-tooltip
                v-for="lang in character.speciesDetails.languages"
                :key="lang.index || lang.name"
                location="top"
                :text="`You can speak, read, and write ${typeof lang === 'string' ? lang : lang.name}`"
              >
                <template #activator="{ props }">
          ...
          <div class="mb-4">
            <div v-if="speciesTraitsWithDetails.length">
              ...
              <div class="trait-accordion">
                <v-card v-for="(trait, index) in speciesTraitsWithDetails" :key="trait.name || trait.index" class="mb-2" variant="outlined">
                  <v-card-title class="pa-3 cursor-pointer trait-header" :class="{ 'trait-expanded': expandedTraits[index] }" @click="toggleTrait(index)">
                    <v-icon class="mr-2">{{ expandedTraits[index] ? 'mdi-chevron-down' : 'mdi-chevron-right' }}</v-icon>
                  ...
                  <v-expand-transition>
                    <v-card-text v-if="expandedTraits[index]" class="pt-0">
                      ...
                      <div v-else class="text-caption text-grey">Loading trait details...</div>
                    </v-card-text>
                  </v-expand-transition>
                </v-card>
              </div>
            ...
            <div v-else-if="character.speciesDetails.name === 'Human'">
              ...
              <v-card class="pa-4" color="blue-lighten-5" variant="tonal">
                <div class="text-body-2 mb-3">
                  <strong>Adaptable Nature:</strong>
                  Humans excel through versatility rather than specialized traits:
                </div>
                <v-row>
                  <v-col cols="12" md="6">
                    <v-list density="compact">
                      <v-list-item class="pa-1">
                        <v-icon class="me-3" color="primary" size="small">mdi-account-multiple</v-icon>
                        <v-list-item-title class="text-body-2">Diverse skill development</v-list-item-title>
                      </v-list-item>
                      <v-list-item class="pa-1">
                        <v-icon class="me-3" color="primary" size="small">mdi-star</v-icon>
                  ...
                  <v-col cols="12" md="6">
                    <v-list density="compact">
                      <v-list-item class="pa-1">
                        <v-icon class="me-3" color="primary" size="small">mdi-heart</v-icon>
                        <v-list-item-title class="text-body-2">Cultural adaptability</v-list-item-title>
                      </v-list-item>
                      <v-list-item class="pa-1">
                        <v-icon class="me-3" color="primary" size="small">mdi-lightning-bolt</v-icon>
            ...
            <div v-else-if="character.speciesDetails">
              <h4 class="mb-3">✨ Special Traits</h4>
              <v-alert class="text-body-2" type="info" variant="tonal">
                No special traits found for {{ character.speciesDetails.name }}. This species may have unique abilities that aren't yet loaded.
              </v-alert>
            </div>
          </div>
        </v-card-text>
      ...
      <v-card v-if="character.classDetails" class="mb-6" variant="outlined">
        ...
        <v-card-text>
          ...
          <div class="my-1 ml-4">
            <div class="d-flex flex-wrap align-center ga-2 mb-3">
              <v-tooltip location="top" text="The die used to determine hit points gained each level">
                <template #activator="{ props }">
                  <v-chip v-bind="props" color="error" prepend-icon="mdi-dice-6" size="small">
                    d{{ character.classDetails.hit_die || character.classDetails.hpDie?.replace('D', '') || '8' }} Hit Die
                  </v-chip>
                </template>
              </v-tooltip>
              <v-tooltip location="top" text="The most important ability score(s) for this class">
                <template #activator="{ props }">
                  <v-chip v-bind="props" color="primary" prepend-icon="mdi-star" size="small">
                    {{
              ...
              >
                <template #activator="{ props }">
                  <v-chip v-bind="props" color="warning" prepend-icon="mdi-shield-check" size="small">
          ...
          <div v-if="character.classDetails.features?.length" class="mb-4">
            ...
            <div class="class-features-accordion">
              <v-card v-for="(feature, index) in character.classDetails.features" :key="feature.name || feature.index" class="mb-2" variant="outlined">
                <v-card-title class="pa-3 cursor-pointer feature-header" :class="{ 'feature-expanded': expandedClassFeatures[index] }" @click="toggleClassFeature(index)">
                  <v-icon class="mr-2">{{ expandedClassFeatures[index] ? 'mdi-chevron-down' : 'mdi-chevron-right' }}</v-icon>
                ...
                <v-expand-transition>
                  <v-card-text v-if="expandedClassFeatures[index]" class="pt-0">
                    ...
                    <div v-else class="text-caption text-grey">This feature provides special abilities for your {{ character.classDetails.name }}.</div>
                  </v-card-text>
                </v-expand-transition>
              </v-card>
            </div>
          ...
          <div v-else class="mb-4">
            <h4 class="mb-3">✨ 1st Level Features</h4>
            <v-alert class="text-body-2" type="info" variant="tonal">
              Class features are loading... Each {{ character.classDetails.name }} gets unique abilities at 1st level that define how the class plays.
            </v-alert>
          </div>
        </v-card-text>
      </v-card>
          ...
          <v-row>
            <v-col cols="12" md="6">
              <div v-if="character.backgroundDetails.skill_proficiencies?.length" class="mb-4">
                ...
                <v-chip-group>
                  <v-tooltip v-for="skill in character.backgroundDetails.skill_proficiencies" :key="skill.name" location="top" :text="`Your background grants proficiency in ${skill.name}`">
                    <template #activator="{ props }">
              ...
              <div v-if="character.backgroundDetails.languages?.length" class="mb-4">
                ...
                <v-chip-group>
                  <v-tooltip
                    v-for="lang in character.backgroundDetails.languages"
                    :key="lang.index || lang.name"
                    location="top"
              ...
              <div v-if="backgroundLanguageChoices > 0" class="mb-4">
                ...
                <v-autocomplete
                  ...
                  :items="availableLanguageChoices"
                  :label="`Choose ${backgroundLanguageChoices} languages`"
                  multiple
              ...
              <div v-if="backgroundToolChoices > 0" class="mb-4">
                ...
                <v-autocomplete
                  ...
                  chips
                  :counter="backgroundToolChoices"
                  :items="availableToolChoices"
                  :label="`Choose ${backgroundToolChoices} tools`"
                  multiple
      ...
      <v-card class="mb-6" variant="outlined">
        ...
        <v-card-text>
          ...
          <div v-if="!hasFightingStyle && !hasExpertise && !isWarlock && !isDruid && !isCleric" class="text-center py-6">
            <v-icon color="grey-lighten-2" size="64">mdi-cog-outline</v-icon>
            <div class="text-h6 mt-3 mb-2">No Feature Choices Available</div>
            <div class="text-body-2 text-medium-emphasis mb-2">Select a class to see available feature customization options.</div>
            <v-chip color="info" size="small" variant="outlined">Current class: {{ character.classDetails?.name || character.class || 'None selected' }}</v-chip>
          </div>
            ...
            <v-col v-if="hasFightingStyle" cols="12">
              <v-card class="feature-choice-card" color="red-lighten-5" variant="tonal">
                <v-card-title class="d-flex align-center text-h6">
                  ...
                  Fighting Style
                  <v-spacer />
                  <v-chip :color="hasFightingStyle && !selectedFightingStyle ? 'error' : 'red'" size="small" variant="outlined">
                    {{ hasFightingStyle && !selectedFightingStyle ? 'Required!' : 'Required' }}
                  </v-chip>
                </v-card-title>
                <v-card-text>
                  <div class="text-body-2 mb-3 text-medium-emphasis">Choose a fighting style that defines your combat approach</div>
                        ...
                        >
                          <v-card-text class="d-flex pa-3" style="min-height: 80px">
                            <div class="radio-container">
                              <v-radio hide-details :value="style.value" />
                            </div>
            ...
            <v-col v-if="hasExpertise" cols="12">
              <v-card class="feature-choice-card" color="yellow-lighten-5" variant="tonal">
                <v-card-title class="d-flex align-center text-h6">
                  ...
                  Expertise
                  <v-spacer />
                  <v-chip :color="selectedExpertiseSkills.length < expertiseChoices ? 'error' : 'yellow-darken-2'" size="small" variant="outlined">
                    {{
                      selectedExpertiseSkills.length < expertiseChoices
                        ? `Required:
                      ${selectedExpertiseSkills.length}/${expertiseChoices}`
                        : `Choose ${expertiseChoices}`
                    }}
                  </v-chip>
                </v-card-title>
                <v-card-text>
                  <div class="text-body-2 mb-3 text-medium-emphasis">Choose {{ expertiseChoices }} skills to double your proficiency bonus</div>
                  ...
                  <div v-if="selectedExpertiseSkills.length" class="mb-3">
                    ...
                    <div class="d-flex flex-wrap ga-2">
                      <v-chip
                        v-for="skill in selectedExpertiseSkills"
                        :key="`selected-${skill}`"
                        closable
                  ...
                  <div class="expertise-skills-grid">
                    <!-- Strength Skills -->
                    <div v-if="strengthSkills.length > 0" class="skill-category" :class="{ 'primary-ability': isImportantAbility('Strength') }">
                      <div class="skill-category-header">
                        <v-icon class="me-2" :color="isImportantAbility('Strength') ? 'red-darken-1' : 'red-darken-3'" size="small">mdi-arm-flex</v-icon>
                        <span class="text-subtitle-2 font-weight-bold">Strength</span>
                        <v-chip v-if="isImportantAbility('Strength')" class="ml-2" color="primary" size="x-small" variant="outlined">Key Ability</v-chip>
                        <v-spacer />
                        <span class="text-caption">{{ getAbilityModifier('strength') >= 0 ? '+' : '' }}{{ getAbilityModifier('strength') }}</span>
                      </div>
                      <div class="skill-chips">
                        <v-tooltip v-for="skill in strengthSkills" :key="skill" location="top" :text="`Double your proficiency bonus for ${skill} checks (uses Strength)`">
                          <template #activator="{ props }">
                            <v-chip
                              v-bind="props"
                              :color="selectedExpertiseSkills.includes(skill) ? (isImportantAbility('Strength') ? 'red-darken-1' : 'red-lighten-1') : 'grey-lighten-1'"
                    ...
                    <!-- Dexterity Skills -->
                    <div v-if="dexteritySkills.length > 0" class="skill-category" :class="{ 'primary-ability': isImportantAbility('Dexterity') }">
                      <div class="skill-category-header">
                        <v-icon class="me-2" :color="isImportantAbility('Dexterity') ? 'green-darken-1' : 'green-darken-3'" size="small">mdi-run-fast</v-icon>
                        <span class="text-subtitle-2 font-weight-bold">Dexterity</span>
                        <v-chip v-if="isImportantAbility('Dexterity')" class="ml-2" color="primary" size="x-small" variant="outlined">Key Ability</v-chip>
                        <v-spacer />
                        <span class="text-caption">{{ getAbilityModifier('dexterity') >= 0 ? '+' : '' }}{{ getAbilityModifier('dexterity') }}</span>
                      </div>
                      <div class="skill-chips">
                        <v-tooltip v-for="skill in dexteritySkills" :key="skill" location="top" :text="`Double your proficiency bonus for ${skill} checks (uses Dexterity)`">
                          <template #activator="{ props }">
                            <v-chip
                              v-bind="props"
                              :color="selectedExpertiseSkills.includes(skill) ? (isImportantAbility('Dexterity') ? 'green-darken-1' : 'green-lighten-1') : 'grey-lighten-1'"
                              :disabled="!selectedExpertiseSkills.includes(skill) && selectedExpertiseSkills.length >= expertiseChoices"
                              size="small"
                              :value="skill"
                              variant="outlined"
                    ...
                    <!-- Intelligence Skills -->
                    <div v-if="intelligenceSkills.length > 0" class="skill-category" :class="{ 'primary-ability': isImportantAbility('Intelligence') }">
                      <div class="skill-category-header">
                        <v-icon class="me-2" :color="isImportantAbility('Intelligence') ? 'blue-darken-1' : 'blue-darken-3'" size="small">mdi-brain</v-icon>
                        <span class="text-subtitle-2 font-weight-bold">Intelligence</span>
                        <v-chip v-if="isImportantAbility('Intelligence')" class="ml-2" color="primary" size="x-small" variant="outlined">Key Ability</v-chip>
                        <v-spacer />
                        <span class="text-caption">{{ getAbilityModifier('intelligence') >= 0 ? '+' : '' }}{{ getAbilityModifier('intelligence') }}</span>
                      </div>
                      <div class="skill-chips">
                        <v-tooltip v-for="skill in intelligenceSkills" :key="skill" location="top" :text="`Double your proficiency bonus for ${skill} checks (uses Intelligence)`">
                          <template #activator="{ props }">
                            <v-chip
                              v-bind="props"
                              :color="selectedExpertiseSkills.includes(skill) ? (isImportantAbility('Intelligence') ? 'blue-darken-1' : 'blue-lighten-1') : 'grey-lighten-1'"
                              :disabled="!selectedExpertiseSkills.includes(skill) && selectedExpertiseSkills.length >= expertiseChoices"
                              size="small"
                              :value="skill"
                              variant="outlined"
                    ...
                    <!-- Wisdom Skills -->
                    <div v-if="wisdomSkills.length > 0" class="skill-category" :class="{ 'primary-ability': isImportantAbility('Wisdom') }">
                      <div class="skill-category-header">
                        <v-icon class="me-2" :color="isImportantAbility('Wisdom') ? 'teal-darken-1' : 'teal-darken-3'" size="small">mdi-eye</v-icon>
                        <span class="text-subtitle-2 font-weight-bold">Wisdom</span>
                        <v-chip v-if="isImportantAbility('Wisdom')" class="ml-2" color="primary" size="x-small" variant="outlined">Key Ability</v-chip>
                        <v-spacer />
                        <span class="text-caption">{{ getAbilityModifier('wisdom') >= 0 ? '+' : '' }}{{ getAbilityModifier('wisdom') }}</span>
                      </div>
                      <div class="skill-chips">
                        <v-tooltip v-for="skill in wisdomSkills" :key="skill" location="top" :text="`Double your proficiency bonus for ${skill} checks (uses Wisdom)`">
                          <template #activator="{ props }">
                            <v-chip
                              v-bind="props"
                              :color="selectedExpertiseSkills.includes(skill) ? (isImportantAbility('Wisdom') ? 'teal-darken-1' : 'teal-lighten-1') : 'grey-lighten-1'"
                              :disabled="!selectedExpertiseSkills.includes(skill) && selectedExpertiseSkills.length >= expertiseChoices"
                              size="small"
                              :value="skill"
                              variant="outlined"
                    ...
                    <!-- Charisma Skills -->
                    <!-- Charisma Skills -->
                    <div v-if="charismaSkills.length > 0" class="skill-category" :class="{ 'primary-ability': isImportantAbility('Charisma') }">
                      <div class="skill-category-header">
                        <v-icon class="me-2" :color="isImportantAbility('Charisma') ? 'purple-darken-1' : 'purple-darken-3'" size="small">mdi-account-group</v-icon>
                        <span class="text-subtitle-2 font-weight-bold">Charisma</span>
                        <v-chip v-if="isImportantAbility('Charisma')" class="ml-2" color="primary" size="x-small" variant="outlined">Key Ability</v-chip>
                        <v-spacer />
                        <span class="text-caption">{{ getAbilityModifier('charisma') >= 0 ? '+' : '' }}{{ getAbilityModifier('charisma') }}</span>
                      </div>
                      <div class="skill-chips">
                        <v-tooltip v-for="skill in charismaSkills" :key="skill" location="top" :text="`Double your proficiency bonus for ${skill} checks (uses Charisma)`">
                          <template #activator="{ props }">
                            <v-chip
                              v-bind="props"
                              :color="selectedExpertiseSkills.includes(skill) ? (isImportantAbility('Charisma') ? 'purple-darken-1' : 'purple-lighten-1') : 'grey-lighten-1'"
                              :disabled="!selectedExpertiseSkills.includes(skill) && selectedExpertiseSkills.length >= expertiseChoices"
                              size="small"
                              :value="skill"
                              variant="outlined"
              ...
              <v-row>
                ...
                <v-col v-if="isWarlock" cols="12" md="4">
                  <v-card class="feature-choice-card h-100" color="purple-lighten-5" variant="tonal">
                    ...
                    <v-card-text>
                      <div class="text-caption mb-3 text-medium-emphasis">Choose the entity that grants you power</div>
                      <v-select v-model="selectedPatron" density="compact" item-title="name" item-value="value" :items="warlockPatrons" label="Select your patron" variant="outlined">
                ...
                <v-col v-if="isDruid" cols="12" md="4">
                  <v-card class="feature-choice-card h-100" color="green-lighten-5" variant="tonal">
                    ...
                    <v-card-text>
                      <div class="text-caption mb-3 text-medium-emphasis">Choose your connection to nature</div>
                      <v-select v-model="selectedCircle" density="compact" item-title="name" item-value="value" :items="druidCircles" label="Select your circle" variant="outlined">
                        <template #item="{ props, item }">
                          <v-list-item v-bind="props">
                            <v-list-item-title>{{ item.raw.name }}</v-list-item-title>
                            <v-list-item-subtitle class="text-caption">{{ item.raw.description }}</v-list-item-subtitle>
                ...
                <v-col v-if="isCleric" cols="12" md="4">
                  <v-card class="feature-choice-card h-100" color="blue-lighten-5" variant="tonal">
                    ...
                    <v-card-text>
                      <div class="text-caption mb-3 text-medium-emphasis">Choose your deity's sphere of influence</div>
                      <v-select v-model="selectedDomain" density="compact" item-title="name" item-value="value" :items="clericDomains" label="Select your domain" variant="outlined">
                        <template #item="{ props, item }">
                          <v-list-item v-bind="props">
                            <v-list-item-title>{{ item.raw.name }}</v-list-item-title>
                            <v-list-item-subtitle class="text-caption">{{ item.raw.description }}</v-list-item-subtitle>
      ...
      <v-card class="mb-6" variant="outlined">
        ...
        <v-card-text>
              ...
              <v-card class="skill-card" color="blue-lighten-5" variant="tonal">
                <v-card-text class="py-2">
                  <div class="d-flex align-center">
                    ...
                    <v-spacer />
                    <v-tooltip location="top" :text="`Based on ${skill.ability} modifier`">
                      <template #activator="{ props }">
                    ...
                    <v-tooltip location="top" :text="`Total bonus: ability modifier + proficiency bonus`">
                      <template #activator="{ props }">
                        <v-chip v-bind="props" class="ml-1" color="success" size="x-small">+{{ calculateSkillBonus(skill) }}</v-chip>
                      </template>
                    </v-tooltip>
                  </div>
          ...
          <div v-else class="text-center text-medium-emphasis py-4">Select a class and background to see available skills.</div>
        </v-card-text>
      </v-card>
      ...
      <v-card v-if="isSpellcaster" class="mb-6" variant="outlined">
        ...
        <v-card-text>
          ...
          <div v-else>
            <v-tabs v-model="spellTab" class="mb-2" color="accent" grow>
            ...
            <v-window v-model="spellTab">
              ...
              <v-window-item v-if="cantripsKnown > 0" :value="0">
                <v-autocomplete
                  ...
                  item-title="name"
                  item-value="index"
                  :items="characterStore.availableCantrips"
                  :label="`Choose up to ${cantripsKnown} cantrips`"
                  multiple
                  :rules="[(v) => v.length <= cantripsKnown || `Select up to ${cantripsKnown}`]"
                  @update:model-value="onCantripSelect"
                />
                <v-list v-if="selectedCantrips.length">
                  <v-list-item v-for="spell in selectedCantripsDetails" :key="spell.index" :active="selectedCantripDetail === spell.index" @click="selectedCantripDetail = spell.index">
              ...
              <v-window-item v-if="spellsKnown > 0" :value="1">
                <v-autocomplete
                  ...
                  item-title="name"
                  item-value="index"
                  :items="characterStore.availableSpells"
                  :label="`Choose up to ${spellsKnown} 1st-level spells`"
                  multiple
                  :rules="[(v) => v.length <= spellsKnown || `Select up to ${spellsKnown}`]"
                  @update:model-value="onSpellSelect"
                />
                <v-list v-if="selectedSpells.length">
                  <v-list-item v-for="spell in selectedSpellsDetails" :key="spell.index" :active="selectedSpellDetail === spell.index" @click="selectedSpellDetail = spell.index">
                    <v-list-item-title>{{ spell.name }}</v-list-item-title>
                  </v-list-item>
                </v-list>
                <v-expand-transition>
...
const availableClassSkills = computed(() => {
  ...
  const skillChoice = character.value.classDetails.proficiency_choices.find((choice) => choice.type === 'proficiencies' && choice.from?.some((opt) => opt.proficiency?.type === 'skills'))

  if (!skillChoice) return []

  return skillChoice.from?.filter((opt) => opt.proficiency?.type === 'skills')?.map((opt) => opt.proficiency.name.replace('Skill: ', '')) || []
})
...
const getAllSkillProficiencies = computed(() => {
  ...
  if (character.value.backgroundDetails?.name?.toLowerCase().includes('soldier') || character.value.background?.toLowerCase().includes('soldier')) {
...
const calculateSkillBonus = (skill) => {
  ...
  switch (skill.ability) {
    case 'Strength':
      abilityMod = getStrModifier.value
      break
    case 'Dexterity':
      abilityMod = getDexModifier.value
      break
    case 'Constitution':
      abilityMod = getConModifier.value
      break
    case 'Intelligence':
      abilityMod = getIntModifier.value
      break
    case 'Wisdom':
      abilityMod = getWisModifier.value
      break
    case 'Charisma':
      abilityMod = getChaModifier.value
      break
  }
...
const availableLanguageChoices = ['Abyssal', 'Celestial', 'Draconic', 'Deep Speech', 'Infernal', 'Primordial', 'Sylvan', 'Undercommon', 'Giant', 'Goblin', 'Halfling', 'Orc', 'Gnomish', 'Dwarvish']
...
const hasFightingStyle = computed(() => {
  ...
  return fightingStyleClasses.includes(character.value.class) || fightingStyleClasses.includes(character.value.classDetails?.index) || fightingStyleClasses.includes(character.value.classDetails?.id)
})

const hasExpertise = computed(() => {
  const expertiseClasses = ['rogue', 'bard']
  return expertiseClasses.includes(character.value.class) || expertiseClasses.includes(character.value.classDetails?.index) || expertiseClasses.includes(character.value.classDetails?.id)
})
...
const isImportantAbility = (abilityName) => {
  ...
  const primaryAbilities = character.value.classDetails.primary_ability || character.value.classDetails.primaryAbility || []
...
watch(
  [selectedFightingStyle, selectedExpertiseSkills, selectedPatron, selectedCircle, selectedDomain],
  () => {
