<template>
  <v-card flat>
    ...
    <div class="d-flex justify-center mb-6">
      <v-btn-toggle v-model="equipmentMethod" class="custom-toggle elevation-2 rounded-pill pa-1" color="primary" mandatory>
        <v-btn :class="['toggle-btn', equipmentMethod === 'package' ? 'selected-package' : '']" style="margin-right: 12px" value="package" variant="flat">Standard Package</v-btn>
        <v-btn :class="['toggle-btn', equipmentMethod === 'gold' ? 'selected-gold' : '']" value="gold" variant="flat">Buy with Gold</v-btn>
      </v-btn-toggle>
    </div>
    ...
    <template v-if="equipmentMethod === 'package'">
      <div>
        ...
        <v-card class="mb-6" variant="outlined">
          ...
          <v-card-subtitle class="text-caption">Choose your starting equipment for {{ capitalizeClassName(character?.selectedClass?.name || character?.class || 'your class') }}</v-card-subtitle>
          <v-card-text>
              ...
              <div v-for="(choice, index) in equipmentChoices" :key="'choice-' + index" class="mb-6">
                ...
                <div class="d-flex flex-wrap ga-3">
                  <template v-for="(option, optionIndex) in choice.options" :key="'option-' + optionIndex">
                    <v-tooltip v-if="option.items && option.items.length === 1 && getEquipmentTooltip(option.items[0].name)" location="top" max-width="300">
                      <template #activator="{ props: tooltipProps }">
                        <v-chip
                          v-bind="tooltipProps"
                          class="equipment-choice-chip"
                          :clickable="choice.options.length > 1"
                      ...
                      <template #default>
                        <div class="equipment-tooltip">
                          ...
                          <div class="text-caption mb-1">
                            <strong>Type:</strong>
                            {{ getEquipmentTooltip(option.items[0].name).type }}
                          </div>
                          <div v-if="getEquipmentTooltip(option.items[0].name).damage" class="text-caption mb-1">
                            <strong>Damage:</strong>
                            {{ getEquipmentTooltip(option.items[0].name).damage }}
                          </div>
                          <div v-if="getEquipmentTooltip(option.items[0].name).ac" class="text-caption mb-1">
                            <strong>AC:</strong>
                            {{ getEquipmentTooltip(option.items[0].name).ac }}
                          </div>
                          <div v-if="getEquipmentTooltip(option.items[0].name).properties !== 'None'" class="text-caption mb-1">
                            <strong>Properties:</strong>
                            {{ getEquipmentTooltip(option.items[0].name).properties }}
                          </div>
                          ...
                          <div v-if="getEquipmentTooltip(option.items[0].name).contents" class="mt-2">
                            ...
                            <div class="equipment-pack-contents">
                              <div v-for="item in getEquipmentTooltip(option.items[0].name).contents" :key="item" class="text-caption pack-item">• {{ item }}</div>
                            </div>
                          </div>
                          <div v-if="!getEquipmentTooltip(option.items[0].name).contents" class="text-caption text-grey-darken-1 mt-2" style="font-style: italic">
            ...
            <div v-else-if="fallbackEquipment.length > 0">
              <v-alert class="mb-4" type="info" variant="tonal">
                ...
                No equipment choices found for {{ character?.selectedClass?.name || 'this class' }}. Showing standard equipment.
              </v-alert>
              <v-row>
                <v-col v-for="item in fallbackEquipment" :key="item.name" cols="12" lg="4" md="6">
            ...
            <div v-else class="text-center py-8">
              <v-icon color="grey" size="64">mdi-package-variant-closed</v-icon>
        ...
        <v-card v-if="backgroundEquipmentChoices.length > 0" class="mb-6" variant="outlined">
          ...
          <v-card-subtitle class="text-caption">Choose your starting equipment for {{ character?.backgroundDetails?.name || character?.background || 'your background' }}</v-card-subtitle>
          <v-card-text>
            <div class="d-flex flex-wrap ga-2">
              <div v-for="item in backgroundEquipmentChoices" :key="item?.name || 'unknown'" class="background-equipment-item">
                <!-- Musical Instrument Chip with Click Functionality -->
                <template v-if="item && item.name && (item.name?.toLowerCase().includes('musical instrument') || item.name?.toLowerCase() === 'instrument' || isSpecificMusicalInstrument(item))">
                  <v-tooltip location="top" :text="isGenericMusicalInstrument(item) ? 'Click to choose a specific musical instrument' : 'Click to change musical instrument'">
                    <template #activator="{ props: tooltipProps }">
                      <v-chip
                        v-bind="tooltipProps"
                        class="ma-1"
                        ...
                        :prepend-icon="isChoiceRequiredItem(item) ? 'mdi-help-circle' : 'mdi-music'"
                        size="large"
                        ...
                        @click="musicalInstrumentDialog = true"
                      />
                    </template>
                  </v-tooltip>
                </template>
                ...
                <template v-if="item && item.name && (item.name?.toLowerCase().includes('artisan') || item.name?.toLowerCase().includes('artisan\'s tools') || isSpecificArtisanTool(item))">
                  <v-tooltip location="top" :text="isGenericArtisanTool(item) ? 'Click to choose specific artisan\'s tools' : 'Click to change artisan\'s tools'">
                    <template #activator="{ props: tooltipProps }">
                      <v-chip
                        v-bind="tooltipProps"
                        class="ma-1"
                  ...
                  <v-tooltip v-if="getEquipmentTooltip(item.name)" location="top" max-width="300">
                    ...
                    <template #default>
                      <div class="equipment-tooltip">
                        ...
                        <div class="text-caption mb-1">
                          <strong>Type:</strong>
                          {{ getEquipmentTooltip(item.name).type }}
                        </div>
                        <div v-if="getEquipmentTooltip(item.name).damage" class="text-caption mb-1">
                          <strong>Damage:</strong>
                          {{ getEquipmentTooltip(item.name).damage }}
                        </div>
                        <div v-if="getEquipmentTooltip(item.name).ac" class="text-caption mb-1">
                          <strong>AC:</strong>
                          {{ getEquipmentTooltip(item.name).ac }}
                        </div>
                        <div v-if="getEquipmentTooltip(item.name).properties !== 'None'" class="text-caption mb-1">
                          <strong>Properties:</strong>
                          {{ getEquipmentTooltip(item.name).properties }}
                        </div>
                        <div v-if="getEquipmentTooltip(item.name).contents" class="mt-2">
                          <div class="text-caption font-weight-bold mb-1 text-primary">Pack Contents:</div>
                          <div class="equipment-pack-contents">
                            <div v-for="packItem in getEquipmentTooltip(item.name).contents" :key="packItem" class="text-caption pack-item">• {{ packItem }}</div>
                          </div>
                        </div>
                        <div v-if="!getEquipmentTooltip(item.name).contents" class="text-caption text-grey-darken-1 mt-2" style="font-style: italic">
        ...
        <v-card class="mt-2" variant="outlined">
          ...
          <v-card-text>
            ...
            <div v-else class="d-flex flex-wrap ga-2">
              <div v-for="(item, index) in allInventory" :key="`${item?.name || 'unknown'}-${item?.source || 'unknown'}-${index}`" class="inventory-equipment-item">
                <!-- Musical Instrument Chip -->
                <template v-if="item && item.name && isSpecificMusicalInstrument(item)">
                  <v-tooltip location="top" :text="`Musical instrument from your ${item.source === 'class' ? 'class equipment' : 'background'}`">
                    <template #activator="{ props: tooltipProps }">
                      <v-chip
                        v-bind="tooltipProps"
                        class="ma-1"
                ...
                <template v-else-if="item && item.name && isSpecificArtisanTool(item)">
                  <v-tooltip location="top" :text="`Artisan's tools from your ${item.source === 'background' ? 'background' : 'class equipment'}`">
                    <template #activator="{ props: tooltipProps }">
                      <v-chip
                        v-bind="tooltipProps"
                        class="ma-1"
    ...
    <template v-else-if="equipmentMethod === 'gold'">
      <div>
        ...
        <div class="d-flex flex-wrap justify-center mb-4">
          <v-chip class="ma-1" color="warning" size="large" variant="tonal">💰 Gold: {{ characterStore.character?.coins?.gp || 0 }} gp</v-chip>
          ...
          <v-col cols="12" md="4">
            <v-card class="equipment-shop-category mb-4" variant="outlined">
              <v-card-title class="d-flex align-center bg-red-lighten-5 text-red-darken-2">
                ...
                <span>Weapons</span>
                <v-spacer />
                <v-chip color="red" size="small" variant="text">{{ availableWeapons.length }}</v-chip>
              </v-card-title>
              <v-card-text class="pa-4">
                <div v-if="availableWeapons.length === 0" class="text-center text-grey py-4">No weapons available</div>
                <div v-else class="text-center">
                  <div class="text-caption text-grey-darken-1 mb-3">Browse {{ availableWeapons.length }} available weapons</div>
                  <v-btn block color="red" size="large" variant="tonal" @click="addWeaponDialog = true">
          ...
          <v-col cols="12" md="4">
            <v-card class="equipment-shop-category mb-4" variant="outlined">
              <v-card-title class="d-flex align-center bg-blue-lighten-5 text-blue-darken-2">
                ...
                <span>Armor</span>
                <v-spacer />
                <v-chip color="blue" size="small" variant="text">{{ availableArmor.length }}</v-chip>
              </v-card-title>
              <v-card-text class="pa-4">
                <div v-if="availableArmor.length === 0" class="text-center text-grey py-4">No armor available</div>
                <div v-else class="text-center">
                  <div class="text-caption text-grey-darken-1 mb-3">Browse {{ availableArmor.length }} available armor sets</div>
                  <v-btn block color="blue" size="large" variant="tonal" @click="addArmorDialog = true">
                    <v-icon start>mdi-storefront</v-icon>
                    Browse Armor Shop
                  </v-btn>
                </div>
          ...
          <v-col cols="12" md="4">
            <v-card class="equipment-shop-category mb-4" variant="outlined">
              <v-card-title class="d-flex align-center bg-green-lighten-5 text-green-darken-2">
                ...
                <span>Gear</span>
                <v-spacer />
                <v-chip color="green" size="small" variant="text">{{ availableGear.length }}</v-chip>
              </v-card-title>
              <v-card-text class="pa-4">
                <div v-if="availableGear.length === 0" class="text-center text-grey py-4">No gear available</div>
                <div v-else class="text-center">
                  <div class="text-caption text-grey-darken-1 mb-3">Browse {{ availableGear.length }} available gear items</div>
                  <v-btn block color="green" size="large" variant="tonal" @click="addGearDialog = true">
                    <v-icon start>mdi-storefront</v-icon>
                    Browse Gear Shop
                  </v-btn>
                </div>
        ...
        <v-card class="mt-2" variant="outlined">
          ...
          <v-card-text>
            <div v-if="purchasedItems.length === 0" class="text-center py-4 text-grey">
              <v-icon color="grey" size="32">mdi-storefront</v-icon>
                ...
                <v-tooltip v-if="getEquipmentTooltip(item.name)" location="top" max-width="300">
                  <template #activator="{ props: tooltipProps }">
                    <v-chip
                      ...
                      :close-size="'20'"
                      :color="getEquipmentTypeColor(item.name, 'purchased')"
                      :prepend-icon="getEquipmentIcon(item.name)"
                      size="large"
                      :text="capitalizeEquipmentName(item.name) + (item.quantity > 1 ? ` x${item.quantity}` : '')"
                      variant="elevated"
                      @click:close="removePurchasedItem(item)"
                    />
                  </template>
                  <template #default>
                    <div class="equipment-tooltip">
  ...
  <v-dialog v-model="showInventoryModal" max-width="600">
    <v-card>
      ...
      <v-card-text>
        ...
        <v-list v-else>
          <template v-for="(item, index) in groupedInventory" :key="`${item?.name || 'unknown'}-${item?.source || 'unknown'}-${index}`">
            <v-list-item v-if="item && item.name">
              <v-list-item-title>{{ capitalizeEquipmentName(item.name) }}</v-list-item-title>
              <v-list-item-subtitle>Qty: {{ item.quantity }} | {{ parseEquipmentCost(item).quantity }} {{ parseEquipmentCost(item).unit }}</v-list-item-subtitle>
            </v-list-item>
          </template>
        </v-list>
      </v-card-text>
  ...
  <v-dialog v-model="weaponAlternativesDialog" max-width="800">
    <v-card>
      ...
      <v-card-subtitle>
        Replace
        <strong>{{ currentItemToReplace?.name }}</strong>
        with a weapon you're proficient with
      </v-card-subtitle>
      <v-card-text>
        <v-row>
          <v-col v-for="weapon in alternativeItems" :key="weapon.name" cols="12" md="6" sm="6">
            <v-card class="weapon-card mb-2" color="red" variant="tonal">
              <v-card-title class="d-flex align-center justify-space-between">
                ...
                <v-chip v-if="equipmentMethod === 'gold'" color="red" size="small" variant="elevated">{{ parseEquipmentCost(weapon).quantity }} gp</v-chip>
              </v-card-title>
              <v-card-subtitle class="text-caption">{{ weapon.weapon_category || 'Weapon' }} • {{ weapon.weapon_range || 'Melee' }}</v-card-subtitle>
              <v-card-text>
                <div class="d-flex align-center justify-space-between mb-2">
                  <span>
                    <strong>Damage:</strong>
                    {{ weapon.damage?.dice || '1d6' }} {{ weapon.damage?.type || 'slashing' }}
                  </span>
  ...
  <v-dialog v-model="armorAlternativesDialog" max-width="800">
    <v-card>
      ...
      <v-card-subtitle>
        Replace
        <strong>{{ currentItemToReplace?.name }}</strong>
        with armor you're proficient with
      </v-card-subtitle>
      <v-card-text>
        <v-row>
          <v-col v-for="armor in alternativeItems" :key="armor.name" cols="12" md="6" sm="6">
            <v-card class="armor-card mb-2" color="blue" variant="tonal">
              <v-card-title class="d-flex align-center justify-space-between">
                ...
                <v-chip v-if="equipmentMethod === 'gold'" color="blue" size="small" variant="elevated">{{ parseEquipmentCost(armor).quantity }} gp</v-chip>
              </v-card-title>
  ...
  <v-dialog v-model="musicalInstrumentDialog" max-width="900">
    <v-card>
      ...
      <v-card-subtitle>Select a musical instrument. Proficient instruments will be highlighted in purple.</v-card-subtitle>
      <v-card-text>
        <v-row>
          <v-col v-for="instrument in musicalInstruments" :key="instrument.name" cols="12" md="6" sm="6">
            <v-card class="instrument-card mb-2" :class="{ 'proficient-instrument': isProficientInstrument(instrument) }" :variant="isProficientInstrument(instrument) ? 'outlined' : 'tonal'">
              <v-card-title class="d-flex align-center justify-space-between">
                <span class="font-weight-bold">{{ instrument.name }}</span>
                <div class="d-flex align-center">
                  <v-chip v-if="isProficientInstrument(instrument)" class="mr-2" color="purple-lighten-1" size="small" variant="elevated">
                    <v-icon size="small" start>mdi-star</v-icon>
                    <strong>Proficient</strong>
                  </v-chip>
                  <v-chip v-if="equipmentMethod === 'gold'" :color="isProficientInstrument(instrument) ? 'purple-lighten-1' : 'secondary'" size="small" variant="elevated">
  ...
  <v-dialog v-model="artisanToolsDialog" max-width="900">
    <v-card>
      ...
      <v-card-subtitle>Select artisan's tools. Proficient tools will be highlighted in green.</v-card-subtitle>
      <v-card-text>
        <v-row>
          <v-col v-for="tool in artisanTools" :key="tool.name" cols="12" md="6" sm="6">
            <v-card class="tool-card mb-2" :class="{ 'proficient-tool': isProficientTool(tool) }" :variant="isProficientTool(tool) ? 'outlined' : 'tonal'">
              <v-card-title class="d-flex align-center justify-space-between">
                <span class="font-weight-bold">{{ tool.name }}</span>
                <div class="d-flex align-center">
                  <v-chip v-if="isProficientTool(tool)" class="mr-2" color="green-darken-1" size="small" variant="elevated">
                    <v-icon size="small" start>mdi-star</v-icon>
                    <strong>Proficient</strong>
                  </v-chip>
                  <v-chip v-if="equipmentMethod === 'gold'" :color="isProficientTool(tool) ? 'green-darken-1' : 'secondary'" size="small" variant="elevated">
  ...
  <v-dialog v-model="weaponCategoryDialog" max-width="800">
    <v-card>
      ...
      <v-card-text>
        ...
        <v-row>
          <v-col v-for="weapon in availableWeaponsForSelection" :key="weapon.name" cols="12" lg="4" md="6">
            <v-card
              :class="['weapon-selection-card', { selected: selectedWeapons.includes(weapon.name) }]"
            ...
            >
              <v-card-title class="text-subtitle-1">
                {{ weapon.name }}
                <v-icon v-if="selectedWeapons.includes(weapon.name)" class="ml-2" color="white">mdi-check-circle</v-icon>
              </v-card-title>
              <v-card-text class="pt-2">
                <div class="text-caption">{{ weapon.description }}</div>
                <v-chip v-if="equipmentMethod === 'gold'" class="mt-1" size="x-small" variant="outlined">{{ weapon.cost.quantity }} {{ weapon.cost.unit }}</v-chip>
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn color="primary" :disabled="selectedWeapons.length !== weaponSelectionCount" @click="confirmWeaponSelection">Confirm Selection</v-btn>
  ...
  <v-dialog v-model="addWeaponDialog" max-width="1200" scrollable>
    <v-card>
      <v-card-title class="bg-red-lighten-5 text-red-darken-2 d-flex align-center">
        ...
        <v-spacer />
        <v-chip color="red" size="small" variant="text">{{ availableWeapons.length }} Items</v-chip>
      </v-card-title>
      <v-card-text class="pa-4" style="max-height: 600px">
        <v-row>
          <v-col v-for="weapon in availableWeapons" :key="weapon.name" cols="12" lg="3" md="4" sm="6">
            <v-tooltip v-if="getEquipmentTooltip(weapon.name)" location="top" :text="getEquipmentTooltip(weapon.name).description">
              <template #activator="{ props }">
                <v-card v-bind="props" class="shop-item-card h-100" :class="{ 'elevation-4': canAffordItem(weapon) }" variant="outlined">
                  <v-card-text class="pa-3">
                    <div class="d-flex align-center justify-space-between mb-2">
                      <span class="font-weight-bold text-body-2">{{ capitalizeEquipmentName(weapon.name) }}</span>
                      <v-chip color="red" size="small" variant="elevated">{{ parseEquipmentCost(weapon).quantity }} gp</v-chip>
                    </div>
                    <div class="text-caption text-grey-darken-1 mb-2">{{ weapon.damage?.dice || '1d6' }} {{ weapon.damage?.type || 'damage' }}</div>
                    <div v-if="weapon.weapon_category" class="text-caption mb-2">{{ weapon.weapon_category }} {{ weapon.weapon_range || '' }}</div>
                    <v-btn block color="success" :disabled="!canAffordItem(weapon)" size="small" variant="tonal" @click="openQuantityDialog(weapon)">
            ...
            <v-card v-else class="shop-item-card h-100" :class="{ 'elevation-4': canAffordItem(weapon) }" variant="outlined">
              <v-card-text class="pa-3">
                <div class="d-flex align-center justify-space-between mb-2">
                  <span class="font-weight-bold text-body-2">{{ capitalizeEquipmentName(weapon.name) }}</span>
                  <v-chip color="red" size="small" variant="elevated">{{ parseEquipmentCost(weapon).quantity }} gp</v-chip>
  ...
  <v-dialog v-model="addArmorDialog" max-width="1200" scrollable>
    <v-card>
      <v-card-title class="bg-blue-lighten-5 text-blue-darken-2 d-flex align-center">
        ...
        <v-spacer />
        <v-chip color="blue" size="small" variant="text">{{ availableArmor.length }} Items</v-chip>
      </v-card-title>
      <v-card-text class="pa-4" style="max-height: 600px">
        <v-row>
          <v-col v-for="armor in availableArmor" :key="armor.name" cols="12" lg="3" md="4" sm="6">
            <v-tooltip v-if="getEquipmentTooltip(armor.name)" location="top" :text="getEquipmentTooltip(armor.name).description">
              <template #activator="{ props }">
                <v-card v-bind="props" class="shop-item-card h-100" :class="{ 'elevation-4': canAffordItem(armor) }" variant="outlined">
                  <v-card-text class="pa-3">
                    <div class="d-flex align-center justify-space-between mb-2">
                      <span class="font-weight-bold text-body-2">{{ capitalizeEquipmentName(armor.name) }}</span>
                      <v-chip color="blue" size="small" variant="elevated">{{ parseEquipmentCost(armor).quantity }} gp</v-chip>
                    </div>
                    ...
                    <v-btn block color="success" :disabled="!canAffordItem(armor)" size="small" variant="tonal" @click="openQuantityDialog(armor)">
                      <v-icon size="small" start>mdi-cart</v-icon>
            ...
            <v-card v-else class="shop-item-card h-100" :class="{ 'elevation-4': canAffordItem(armor) }" variant="outlined">
              <v-card-text class="pa-3">
                <div class="d-flex align-center justify-space-between mb-2">
                  <span class="font-weight-bold text-body-2">{{ capitalizeEquipmentName(armor.name) }}</span>
                  <v-chip color="blue" size="small" variant="elevated">{{ parseEquipmentCost(armor).quantity }} gp</v-chip>
  ...
  <v-dialog v-model="addGearDialog" max-width="1200" scrollable>
    <v-card>
      <v-card-title class="bg-green-lighten-5 text-green-darken-2 d-flex align-center">
        ...
        <span>Gear Shop</span>
        <v-spacer />
        <v-chip color="green" size="small" variant="text">{{ availableGear.length }} Items</v-chip>
      </v-card-title>
      <v-card-text class="pa-4" style="max-height: 600px">
        <v-row>
          <v-col v-for="item in availableGear" :key="item.name" cols="12" lg="3" md="4" sm="6">
            <v-tooltip v-if="getEquipmentTooltip(item.name)" location="top" :text="getEquipmentTooltip(item.name).description">
              <template #activator="{ props }">
                <v-card v-bind="props" class="shop-item-card h-100" :class="{ 'elevation-4': canAffordItem(item) }" variant="outlined">
                  <v-card-text class="pa-3">
                    <div class="d-flex align-center justify-space-between mb-2">
                    ...
                    <v-btn block color="success" :disabled="!canAffordItem(item)" size="small" variant="tonal" @click="openQuantityDialog(item)">
                      <v-icon size="small" start>mdi-cart</v-icon>
            ...
            <v-card v-else class="shop-item-card h-100" :class="{ 'elevation-4': canAffordItem(item) }" variant="outlined">
              <v-card-text class="pa-3">
                <div class="d-flex align-center justify-space-between mb-2">
                  <span class="font-weight-bold text-body-2">{{ capitalizeEquipmentName(item.name) }}</span>
                  <v-chip color="green" size="small" variant="elevated">
  ...
  <v-dialog v-model="quantityDialog" max-width="400">
    <v-card>
      ...
      <v-card-text class="pa-4">
        <div v-if="selectedItem" class="text-center">
          <div class="mb-3">
            ...
            <v-chip color="primary" size="small" variant="elevated">{{ formatCostWithGoldEquivalent(selectedItem) }} each</v-chip>
          </div>

          <v-row align="center" class="mb-3" justify="center">
            <v-col cols="4">
              <v-btn color="primary" icon size="large" variant="outlined" @click="selectedQuantity = Math.max(1, selectedQuantity - 1)">
                <v-icon>mdi-minus</v-icon>
              </v-btn>
            </v-col>
            <v-col class="text-center" cols="4">
              <v-text-field v-model.number="selectedQuantity" class="text-center" density="compact" hide-details max="99" min="1" style="text-align: center" type="number" variant="outlined" />
            </v-col>
            <v-col cols="4">
              <v-btn color="primary" icon size="large" variant="outlined" @click="selectedQuantity = Math.min(99, selectedQuantity + 1)">
          ...
          <v-alert v-if="!canAffordQuantity(selectedItem, selectedQuantity)" class="mb-3" type="warning" variant="tonal">
      ...
      <v-card-actions>
        ...
        <v-spacer />
        <v-btn color="success" :disabled="!canAffordQuantity(selectedItem, selectedQuantity)" variant="elevated" @click="purchaseItemWithQuantity()">
...
const capitalizeAndCleanText = (text) => {
  ...
  // Handle "item, descriptor" format (e.g., "crossbow, light" -> "Light Crossbow")
  if (cleanText.includes(',')) {
    const parts = cleanText.split(',').map((part) => part.trim())
    if (parts.length === 2) {
      // Reverse order and capitalize each word
      cleanText = parts
        .reverse()
...
const isChoiceRequiredItem = (item) => {
  ...
  const choiceRequiredItems = ['Musical Instrument', "Artisan's Tools", 'Gaming Set', 'Tool Kit', 'Instrument']
  return choiceRequiredItems.some((choiceItem) => itemName.toLowerCase().includes(choiceItem.toLowerCase()))
}
...
const getEquipmentIcon = (itemName) => {
  ...
  if (
    ...
    name.includes('horn') ||
    name.includes('lyre') ||
    name.includes('pan flute') ||
    name.includes('shawm') ||
    name.includes('viol') ||
    name.includes('bagpipes') ||
    name.includes('guitar') ||
    name.includes('harpsichord') ||
    name.includes('mandolin') ||
  ...
  if (
    name.includes('pack') &&
    (name.includes('dungeoneer') || name.includes('explorer') || name.includes('entertainer') || name.includes('priest') || name.includes('scholar') || name.includes('burglar'))
  ) {
  ...
  if (name.includes('costume') || name.includes('uniform') || name.includes('clothes') || name.includes('vestments')) return 'mdi-tshirt-crew'
  ...
  if (name.includes('book') || name.includes('tome') || name.includes('scroll') || name.includes('parchment') || name.includes('paper')) return 'mdi-book-open'
...
const getEquipmentTypeColor = (itemName, source = null) => {
  ...
  if (name.includes('costume') || name.includes('uniform') || name.includes('clothes') || name.includes('vestments')) {
  ...
  if (name.includes('rations') || name.includes('food') || name.includes('candle') || name.includes('oil') || name.includes('rope') || name.includes('torch')) {
    return 'lime-darken-1'
  }

  // Books and writing materials
  if (name.includes('book') || name.includes('ink') || name.includes('pen') || name.includes('parchment') || name.includes('scroll')) {
  ...
  if (
    ...
    name.includes('smith') ||
    name.includes('brewer') ||
    name.includes('carpenter') ||
    name.includes('leatherworker') ||
    name.includes('mason') ||
    name.includes('potter') ||
    name.includes('weaver') ||
    name.includes('woodcarver')
  ) {
  ...
  if (name.includes('deck') || name.includes('cards') || name.includes('dice') || name.includes('game')) {
...
const equipmentTooltips = ref({
  ...
  "burglar's pack": {
    ...
    properties: 'None',
    description:
      'Contains: Backpack, Ball bearings (bag of 1,000), 10 feet of String, Bell, 5 Candles, Crowbar, Hammer, 10 Pitons, Hooded Lantern, 2 flasks of Oil, 5 days Rations, Tinderbox, Waterskin, 50 feet of Hempen Rope. The pack is strapped to a backpack.',
    contents: [
      'Backpack',
      'Ball bearings (1,000)',
      '10 feet of String',
  ...
  "diplomat's pack": {
    ...
    properties: 'None',
    description:
      'Contains: Chest, 2 cases for maps and scrolls, Set of fine clothes, Bottle of ink, Ink pen, Lamp, 2 flasks of oil, 5 sheets of paper, Vial of perfume, Sealing wax, Soap. The pack is strapped to a backpack.',
...
const classStartingWeapons = computed(() => {
  ...
  const weapons = items
    .filter((item) => {
      ...
      const isWeapon =
        category.includes('weapon') || category.includes('simple') || category.includes('martial') || item.weaponCategory || name.includes('dagger') || name.includes('sword') || name.includes('bow')
...
const startingArmor = computed(() => {
  ...
  const armor = items
    .filter((item) => {
      const category = item.category?.toLowerCase() || ''
      const name = item.name?.toLowerCase() || ''
      const isArmor = category.includes('armor') || item.armorCategory || name.includes('armor') || name.includes('leather') || name.includes('mail') || name.includes('shield')
  ...
  const commonChoices = {
    fighter: [
      {
        ...
        options: [
          {
            items: [{ name: 'Chain Mail', quantity: 1, description: 'Heavy armor, AC 16', cost: { quantity: 75, unit: 'gp' } }],
          },
          {
            items: [
        ...
        options: [
          {
            ...
            choose: 1,
            items: [{ name: 'Shield', quantity: 1, description: '+2 AC', cost: { quantity: 10, unit: 'gp' } }],
          },
          {
            category: 'Martial Weapon',
            choose: 2,
      ...
      },
      {
        ...
        options: [
          ...
          {
            items: [{ name: 'Handaxe', quantity: 2, description: 'Simple weapon, 1d6 slashing, thrown' }],
          },
        ],
      },
      {
        ...
        options: [
          {
            items: [{ name: "Dungeoneer's Pack", quantity: 1, description: 'Backpack, crowbar, hammer, 10 pitons, 10 torches, tinderbox, 10 days of rations, waterskin, 50 feet of hemp rope' }],
          },
          {
            items: [{ name: "Explorer's Pack", quantity: 1, description: 'Backpack, bedroll, mess kit, tinderbox, 10 torches, 10 days of rations, waterskin, 50 feet of hemp rope' }],
          },
        ],
      },
    ],
    rogue: [
      {
        ...
        options: [
          {
            items: [{ name: 'Rapier', quantity: 1, description: 'Martial weapon, 1d8 piercing, finesse' }],
          },
          {
            items: [{ name: 'Shortsword', quantity: 1, description: 'Martial weapon, 1d6 piercing, finesse' }],
          },
        ],
      },
      {
            ...
            items: [
              {
                ...
                quantity: 1,
                description:
                  'Backpack, bag of 1000 ball bearings, 10 feet of string, bell, 5 candles, crowbar, hammer, 10 pitons, hooded lantern, 2 flasks of oil, 5 days rations, tinderbox, waterskin, 50 feet hemp rope',
              },
            ],
          },
          {
    ...
    ranger: [
      {
        ...
        options: [
          {
            items: [{ name: 'Scale Mail', quantity: 1, description: 'Medium armor, AC 14 + Dex modifier (max 2)', cost: { quantity: 50, unit: 'gp' } }],
          },
          {
            items: [{ name: 'Leather Armor', quantity: 1, description: 'Light armor, AC 11 + Dex modifier', cost: { quantity: 10, unit: 'gp' } }],
          },
        ],
      },
      {
        ...
        options: [
          {
            items: [{ name: 'Shortsword', quantity: 2, description: 'Martial weapon, 1d6 piercing, finesse', cost: { quantity: 10, unit: 'gp' } }],
          },
          {
            items: [{ name: 'Handaxe', quantity: 2, description: 'Simple weapon, 1d6 slashing, thrown', cost: { quantity: 5, unit: 'gp' } }],
          },
        ],
      },
      {
            ...
            items: [
              {
                ...
                quantity: 1,
                description: 'Backpack, crowbar, hammer, 10 pitons, 10 torches, tinderbox, 10 days of rations, waterskin, 50 feet of hemp rope',
            ...
            items: [
              {
                ...
                quantity: 1,
                description: 'Backpack, bedroll, mess kit, tinderbox, 10 torches, 10 days of rations, waterskin, 50 feet of hemp rope',
    ...
    wizard: [
      {
        ...
        options: [
          {
            items: [{ name: 'Quarterstaff', quantity: 1, description: 'Simple weapon, 1d6 bludgeoning, versatile (1d8)' }],
          },
          {
            items: [{ name: 'Dagger', quantity: 1, description: 'Simple weapon, 1d4 piercing, finesse, thrown' }],
          },
        ],
      },
      {
        ...
        options: [
          {
            items: [{ name: 'Component Pouch', quantity: 1, description: 'A pouch containing spell components' }],
          },
          {
            items: [{ name: 'Arcane Focus', quantity: 1, description: 'A focus for spellcasting' }],
          },
        ],
      },
      {
        ...
        options: [
          {
            items: [{ name: "Scholar's Pack", quantity: 1, description: 'Backpack, book of lore, bottle of ink, ink pen, 10 sheets of parchment, little bag of sand, small knife' }],
          },
          {
            items: [{ name: "Explorer's Pack", quantity: 1, description: 'Backpack, bedroll, mess kit, tinderbox, 10 torches, 10 days of rations, waterskin, 50 feet of hemp rope' }],
          },
    ...
    paladin: [
      ...
      },
      {
        ...
        options: [
          {
            items: [{ name: 'Javelin', quantity: 5, description: 'Simple weapon, 1d6 piercing, thrown' }],
          },
          {
      ...
      },
      {
        ...
        options: [
          {
            items: [{ name: "Priest's Pack", quantity: 1, description: 'Backpack, blanket, 10 candles, tinderbox, alms box, 2 blocks of incense, censer, vestments, 2 days of rations, waterskin' }],
          },
          {
            items: [{ name: "Explorer's Pack", quantity: 1, description: 'Backpack, bedroll, mess kit, tinderbox, 10 torches, 10 days of rations, waterskin, 50 feet of hemp rope' }],
          },
    ...
    bard: [
      {
        ...
        options: [
          {
            items: [{ name: 'Rapier', quantity: 1, description: 'Martial weapon, 1d8 piercing, finesse', cost: { quantity: 25, unit: 'gp' } }],
          },
          {
            items: [{ name: 'Longsword', quantity: 1, description: 'Martial weapon, 1d8 slashing, versatile', cost: { quantity: 15, unit: 'gp' } }],
          },
          {
            ...
            items: [
              {
                ...
                quantity: 1,
                description: 'Chest, 2 cases for maps and scrolls, fine clothes, bottle of ink, ink pen, lamp, 2 flasks of oil, 5 sheets of paper, vial of perfume, sealing wax, soap',
              },
            ],
          },
          {
            items: [{ name: "Entertainer's Pack", quantity: 1, description: 'Backpack, bedroll, 2 costumes, 5 candles, 5 days of rations, waterskin, disguise kit' }],
          },
        ],
      },
      {
        ...
        options: [
          {
            ...
            choose: 1,
            items: [{ name: 'Musical Instrument', quantity: 1, description: 'Choose any musical instrument', cost: { quantity: 30, unit: 'gp' } }],
          },
        ],
      },
    ],
...
const availableWeapons = computed(() => {
  ...
  if (weapons.length === 0) {
    return characterStore.equipmentData
      .filter((item) => item.name?.toLowerCase().includes('sword') || item.name?.toLowerCase().includes('axe') || item.name?.toLowerCase().includes('bow') || item.weaponCategory)
      .slice(0, 12)
  }
...
const availableArmor = computed(() => {
  ...
  if (armor.length === 0) {
    return characterStore.equipmentData.filter((item) => item.name?.toLowerCase().includes('armor') || item.name?.toLowerCase().includes('shield') || item.armorCategory).slice(0, 12)
  }
...
const defaultClassEquipment = computed(() => {
  ...
  const defaultEquipment = {
    ...
    wizard: [{ name: 'Spellbook', quantity: 1, description: 'Contains your known spells', cost: { quantity: 50, unit: 'gp' } }],
    cleric: [
      { name: 'Scale Mail', quantity: 1, description: 'Medium armor, AC 14 + Dex modifier (max 2)', cost: { quantity: 50, unit: 'gp' } },
    ...
    sorcerer: [{ name: 'Arcane Focus', quantity: 1, description: 'Focus for sorcerous magic', cost: { quantity: 20, unit: 'gp' } }],
    bard: [
      { name: 'Leather Armor', quantity: 1, description: 'Light armor, AC 11 + Dex modifier', cost: { quantity: 10, unit: 'gp' } },
    ...
    monk: [{ name: 'Dart', quantity: 10, description: 'Simple ranged weapon, 1d4 piercing', cost: { quantity: 0.05, unit: 'gp' } }],
    barbarian: [{ name: 'Javelin', quantity: 4, description: 'Simple ranged weapon, 1d6 piercing', cost: { quantity: 0.5, unit: 'gp' } }],
...
const allInventory = computed(() => {
  ...
  if (equipmentMethod.value === 'package') {
    ...
    equipmentChoices.value.forEach((choice, choiceIndex) => {
      ...
      if (selectedOptionIndex !== null && selectedOptionIndex !== undefined) {
        ...
        const customWeapons = characterStore.character?.equipment?.filter((item) => item.choiceIndex === choiceIndex && item.source === 'class') || []
    ...
    console.log(
      ...
      startingEquipmentPackage.value.map((i) => i.name)
    )
    ...
    console.log(
      'Background equipment from computed:',
      backgroundEquipment.value.map((i) => ({ name: i.name, quantity: i.quantity }))
    )
    console.log(
      'Character equipment array:',
      characterStore.character?.equipment?.filter((item) => item.source === 'background')
    )
  ...
  console.log(
    'Raw inventory before filtering:',
    inv.map((i) => ({ name: i.name, source: i.source, quantity: i.quantity }))
  )
  console.log('Raw inventory detailed:', inv)

  // Filter out generic items when specific ones have been selected
  const filteredInv = []
  const hasSpecificMusicalInstrument = inv.some((item) => item.source !== 'class-default' && isSpecificMusicalInstrument(item))
  const hasSpecificArtisanTool = inv.some((item) => item.source !== 'class-default' && isSpecificArtisanTool(item))
  ...
  console.log(
    'All artisan tool related items:',
    artisanToolItems.map((i) => ({ name: i.name, source: i.source, isSpecific: isSpecificArtisanTool(i) }))
  )
  ...
  console.log(
    'Raw inventory before filtering:',
    inv.map((i) => ({ name: i.name, source: i.source }))
  )
  ...
  inv.forEach((item) => {
    ...
    if ((itemName.includes("artisan's tools") || itemName.includes('artisan tools') || itemName === "artisan's tools" || itemName === "set of artisan's tools") && hasSpecificArtisanTool) {
  ...
  console.log(
    'Filtered inventory:',
    filteredInv.map((i) => ({ name: i.name, source: i.source, quantity: i.quantity }))
  )
...
const totalValue = computed(() => {
  ...
  allInventory.value.forEach((item) => {
    ...
    if (!cost) return
    value += cost.unit === 'gp' ? cost.quantity * (item.quantity || 1) : cost.unit === 'sp' ? (cost.quantity / 10) * (item.quantity || 1) : (cost.quantity / 100) * (item.quantity || 1)
  })
...
function showWeaponAlternatives(weapon) {
  ...
  const alternatives = availableWeapons.value.filter((altWeapon) => {
    ...
    if (altWeapon.weapon_category?.toLowerCase().includes('martial')) {
      return proficiencies.some((p) => p.name?.toLowerCase().includes('martial') || p.name?.toLowerCase().includes('all weapons'))
    }

    // Include specific weapon proficiencies
    return proficiencies.some((p) => p.name?.toLowerCase().includes(altWeapon.name?.toLowerCase()) || altWeapon.name?.toLowerCase().includes(p.name?.toLowerCase()))
  })
...
function showArmorAlternatives(armor) {
  ...
  const alternatives = availableArmor.value.filter((altArmor) => {
    ...
    if (altArmor.armor_category?.toLowerCase().includes('medium') || altArmor.armor_category?.toLowerCase().includes('heavy')) {
      return proficiencies.some((p) => p.name?.toLowerCase().includes(altArmor.armor_category?.toLowerCase()) || p.name?.toLowerCase().includes('all armor'))
    }
...
function canAffordQuantity(item, quantity) {
  ...
  const cost = parseEquipmentCost(item)
  const goldCost = cost.unit === 'gp' ? cost.quantity : cost.unit === 'sp' ? cost.quantity / 10 : cost.quantity / 100 // cp to gp
...
function canAffordItem(item) {
  ...
  const goldValue = cost.unit === 'gp' ? cost.quantity : cost.unit === 'sp' ? cost.quantity / 10 : cost.quantity / 100 // cp to gp
...
function selectEquipmentOption(choiceIndex, optionIndex, option) {
    ...
    if (option.category.toLowerCase().includes('melee') || option.category.toLowerCase().includes('weapon')) {
      ...
      if (option.items && option.items.length > 0) {
        ...
        option.items.forEach((item) => {
          ...
          const existingIndex = existingEquipment.findIndex((eq) => eq.name === item.name && eq.choiceIndex === choiceIndex)
  ...
  if (option && option.items) {
    const weaponCategoryItem = option.items.find(
      (item) =>
        item.name &&
...
function confirmWeaponSelection() {
  ...
  if (characterStore.character) {
    ...
    characterStore.character.equipment = characterStore.character.equipment.filter((item) => !(item.source === 'class' && item.choiceIndex === choiceIndex && !item.isFixedItem))
    ...
    if (option && option.fixedItems && option.fixedItems.length > 0) {
      option.fixedItems.forEach((item) => {
        ...
        const existingFixedItem = characterStore.character.equipment.find((eq) => eq.name === item.name && eq.choiceIndex === choiceIndex && eq.isFixedItem)
...
function updateCharacterEquipmentFromChoices() {
  ...
  if (characterStore.character?.equipment) {
    characterStore.character.equipment = characterStore.character.equipment.filter((item) => item.source !== 'class' || (item.source === 'class' && item.choiceIndex !== undefined))
  }
  ...
  equipmentChoices.value.forEach((choice, choiceIndex) => {
    ...
    if (selectedOptionIndex !== null && selectedOptionIndex !== undefined) {
      ...
      const existingCustomWeapons = characterStore.character?.equipment?.filter((item) => item.choiceIndex === choiceIndex && item.optionIndex === selectedOptionIndex && item.source === 'class') || []
      ...
      if (characterStore.character?.equipment) {
        characterStore.character.equipment = characterStore.character.equipment.filter(
          (item) => !(item.choiceIndex === choiceIndex && item.source === 'class' && item.optionIndex !== selectedOptionIndex)
        )
      }
...
const isSpecificMusicalInstrument = (item) => {
  ...
  return specificInstruments.some((instrument) => name.includes(instrument)) && !name.includes('musical instrument') && name !== 'instrument'
}
...
const isProficientInstrument = (instrument) => {
  ...
  if (proficiencies.length > 0) {
    const isProficient = proficiencies.some((prof) => {
      ...
      return profWords.some((profWord) => instrumentWords.some((instWord) => profWord === instWord))
    })
  ...
  if (backgroundDetails) {
    ...
    if (backgroundDetails.selectedInstrument) {
      ...
      if (selectedWords.some((selectedWord) => instrumentWords.some((instWord) => selectedWord === instWord))) {
        return true
      }
    }
    ...
    if (character.selectedBackgroundInstruments && character.selectedBackgroundInstruments.length > 0) {
      ...
      const isSelected = character.selectedBackgroundInstruments.some((selected) => {
        ...
        return selectedWords.some((selectedWord) => instrumentWords.some((instWord) => selectedWord === instWord))
      })
...
const selectMusicalInstrument = (instrument) => {
  ...
  if (isEquipmentChoice) {
    ...
    const filteredEquip = equip.filter((item) => !(item.choiceIndex === choiceIndex && item.optionIndex === optionIndex))
    ...
    console.log(
      '[Instrument] Existing instrument index:',
      idx,
    ...
    requiredBackgroundEquipment.forEach((itemName) => {
      const exists = equip.some((item) => item.name?.toLowerCase() === itemName.toLowerCase() && item.source === 'background')

      if (!exists) {
        console.log('[Instrument] Restoring missing background equipment:', itemName)
        // Proper title case for multi-word items
        const properName = itemName === 'belt pouch' ? 'Belt Pouch' : itemName.charAt(0).toUpperCase() + itemName.slice(1)
        equip.push({
          name: properName,
          quantity: 1,
          equipped: false,
...
const isGenericArtisanTool = (item) => {
  if (!item || !item.name) return false
  const name = item.name.toLowerCase()
  return name.includes("artisan's tools") || name.includes('artisan tools') || name === "artisan's tools" || name === 'artisan tools'
}

const isSpecificArtisanTool = (item) => {
  if (!item || !item.name) return false
  const name = item.name.toLowerCase()
  return artisanTools.value.some((tool) => tool.name.toLowerCase() === name || tool.name.toLowerCase().includes(name) || name.includes(tool.name.toLowerCase()))
}
...
const selectArtisanTool = (tool) => {
  ...
  requiredBackgroundEquipment.forEach((itemName) => {
    ...
    if (!hasItem && itemName !== "set of artisan's tools") {
      // Skip the generic tools since we just replaced it
      const properName = itemName === 'belt pouch' ? 'Belt Pouch' : itemName === 'common clothes' ? 'Common Clothes' : itemName.charAt(0).toUpperCase() + itemName.slice(1)
      equip.push({
        name: properName,
        quantity: 1,
        equipped: false,
...
watch(
  () => character.value?.selectedClass?.id,
