  ...
  <v-card flat>
    ...
    <div class="d-flex justify-center mb-6">
      <v-btn-toggle
        ...
        class="custom-toggle elevation-2 rounded-pill pa-1"
        color="primary"
        mandatory
      >
        <v-btn
          :class="['toggle-btn', equipmentMethod === 'package' ? 'selected-package' : '']"
        ...
        <v-btn :class="['toggle-btn', equipmentMethod === 'gold' ? 'selected-gold' : '']" value="gold" variant="flat">
          Buy with Gold
        </v-btn>
      </v-btn-toggle>
    </div>
        ...
        <v-card class="mb-6" variant="outlined">
          ...
          <v-card-text>
                        ...
                        <div class="equipment-tooltip">
                          ...
                          <div v-if="getEquipmentTooltip(option.items[0].name).contents" class="mt-2">
                            ...
                            <div class="equipment-pack-contents">
                              <div
                                v-for="item in getEquipmentTooltip(option.items[0].name).contents"
                                :key="item"
            ...
            <div v-else-if="fallbackEquipment.length > 0">
              ...
              <v-row>
                <v-col
                  v-for="item in fallbackEquipment"
                  :key="item.name"
                  cols="12"
                  lg="4"
                  md="6"
        ...
        <v-card v-if="backgroundEquipmentChoices.length > 0" class="mb-6" variant="outlined">
          ...
          <v-card-text>
            <div class="d-flex flex-wrap ga-2">
              <div
                v-for="item in backgroundEquipmentChoices"
                    ...
                    <template #default>
                      <div class="equipment-tooltip">
                        ...
                        <div v-if="getEquipmentTooltip(item.name).contents" class="mt-2">
                          ...
                          <div class="equipment-pack-contents">
                            <div
                              v-for="packItem in getEquipmentTooltip(item.name).contents"
                              :key="packItem"
                              class="text-caption pack-item"
                            >
                              • {{ packItem }}
        ...
        <v-card class="mt-2" variant="outlined">
          ...
          <v-card-text>
            ...
            <div v-else class="d-flex flex-wrap ga-2">
              <div
                v-for="(item, index) in allInventory"
  ...
  <v-dialog v-model="showInventoryModal" max-width="600">
    <v-card>
      ...
      <v-card-text>
        ...
        <v-list v-else>
          <template
            v-for="(item, index) in groupedInventory"
            :key="`${item?.name || 'unknown'}-${item?.source || 'unknown'}-${index}`"
          >
  ...
  <v-dialog v-model="weaponAlternativesDialog" max-width="800">
    <v-card>
      ...
      <v-card-text>
        <v-row>
          <v-col
            v-for="weapon in alternativeItems"
            :key="weapon.name"
            cols="12"
            md="6"
            sm="6"
  ...
  <v-dialog v-model="armorAlternativesDialog" max-width="800">
    <v-card>
      ...
      <v-card-text>
        <v-row>
          <v-col
            v-for="armor in alternativeItems"
            :key="armor.name"
            cols="12"
            md="6"
            sm="6"
  ...
  <v-dialog v-model="musicalInstrumentDialog" max-width="900">
    <v-card>
      ...
      <v-card-text>
        <v-row>
          <v-col
            v-for="instrument in musicalInstruments"
            :key="instrument.name"
            cols="12"
            md="6"
            sm="6"
          >
            <v-card
              class="instrument-card mb-2"
              :class="{ 'proficient-instrument': isProficientInstrument(instrument) }"
              :variant="isProficientInstrument(instrument) ? 'outlined' : 'tonal'"
            >
              <v-card-title class="d-flex align-center justify-space-between">
                <span class="font-weight-bold">{{ instrument.name }}</span>
                <div class="d-flex align-center">
                  <v-chip
                    v-if="isProficientInstrument(instrument)"
                    class="mr-2"
                  ...
                  <v-chip
                    v-if="equipmentMethod === 'gold'"
                    :color="isProficientInstrument(instrument) ? 'purple-lighten-1' : 'secondary'"
                    size="small"
                    variant="elevated"
                  >
    ...
    <v-card>
      ...
      <v-card-text>
        <v-row>
          ...
          >
            <v-card
              class="tool-card mb-2"
              :class="{ 'proficient-tool': isProficientTool(tool) }"
              :variant="isProficientTool(tool) ? 'outlined' : 'tonal'"
            >
              <v-card-title class="d-flex align-center justify-space-between">
                <span class="font-weight-bold">{{ tool.name }}</span>
                <div class="d-flex align-center">
                  <v-chip
                    v-if="isProficientTool(tool)"
                    class="mr-2"
                  ...
                  <v-chip
                    v-if="equipmentMethod === 'gold'"
                    :color="isProficientTool(tool) ? 'green-darken-1' : 'secondary'"
                    size="small"
                    variant="elevated"
                  >
  ...
  <v-dialog v-model="weaponCategoryDialog" max-width="800">
    <v-card>
      ...
      <v-card-text>
        ...
        <v-row>
          <v-col
            v-for="weapon in availableWeaponsForSelection"
            :key="weapon.name"
            cols="12"
            lg="4"
            md="6"
          >
            ...
            >
              <v-card-title class="text-subtitle-1">
                {{ weapon.name }}
                <v-icon
                  v-if="selectedWeapons.includes(weapon.name)"
                  class="ml-2"
                  color="white"
              ...
              <v-card-text class="pt-2">
                ...
                <v-chip v-if="equipmentMethod === 'gold'" class="mt-1" size="x-small" variant="outlined">{{
    ...
    <v-card>
      ...
      <v-card-text class="pa-4" style="max-height: 600px;">
        <v-row>
          ...
          >
            <v-tooltip
              v-if="getEquipmentTooltip(weapon.name)"
              location="top"
              :text="getEquipmentTooltip(weapon.name).description"
            >
              <template #activator="{ props }">
                <v-card
                  v-bind="props"
                  class="shop-item-card h-100"
                ...
                >
                  <v-card-text class="pa-3">
                    <div class="d-flex align-center justify-space-between mb-2">
                      ...
                      <v-chip color="red" size="small" variant="elevated">
    ...
    <v-card>
      ...
      <v-card-text class="pa-4" style="max-height: 600px;">
        <v-row>
          ...
          >
            <v-tooltip
              v-if="getEquipmentTooltip(armor.name)"
              location="top"
              :text="getEquipmentTooltip(armor.name).description"
            >
              <template #activator="{ props }">
                <v-card
                  v-bind="props"
    ...
    <v-card>
      ...
      <v-card-text class="pa-4" style="max-height: 600px;">
        <v-row>
          ...
          >
            <v-tooltip
              v-if="getEquipmentTooltip(item.name)"
              location="top"
              :text="getEquipmentTooltip(item.name).description"
            >
              <template #activator="{ props }">
                <v-card
                  v-bind="props"
    ...
    <v-card>
      ...
      <v-card-text class="pa-4">
        <div v-if="selectedItem" class="text-center">
          ...
          <v-alert
            v-if="!canAffordQuantity(selectedItem, selectedQuantity)"
            class="mb-3"
            type="warning"
            variant="tonal"
          >
      ...
      <v-card-actions>
        ...
        <v-btn
          color="success"
          :disabled="!canAffordQuantity(selectedItem, selectedQuantity)"
          variant="elevated"
