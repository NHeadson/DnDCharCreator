<template>
  <!-- eslint-disable-next-line vue/max-attributes-per-line, vue/html-closing-bracket-newline -->
  <v-container class="character-creator" fluid>
    <div class="text-center mb-6">
      ...
      <h2 class="text-h6 text-md-h5 mb-6">
        {{ isEditing ? 'Edit your Character' : 'Create your Character - Choose wisely...' }}
      </h2>
    </div>
      ...
      <v-card>
        ...
        <v-card-text class="text-body-1">
          <p class="mb-3">
            ...
            has been {{ isEditing ? 'updated' : 'created' }} successfully!
          </p>
        ...
        <v-card-actions class="pa-4">
          <v-btn variant="outlined" @click="viewCharacters">View All Characters</v-btn>
          <v-spacer />
          <!-- eslint-disable-next-line vue/max-attributes-per-line -->
          <v-btn v-if="!isEditing" color="primary" prepend-icon="mdi-plus" variant="elevated"
            @click="createAnother">Create Another</v-btn>
          <v-btn v-else color="primary" variant="elevated" @click="viewCharacters">Done</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
...
const { character, currentStep } = characterStore
  ...
  try {
    ...
    const characterData = {
      ...
      calculationMetadata: {
        ...
        lineageAbilityBonuses: character.speciesDetails?.lineages?.find(l => l.id === character.speciesLineage)?.abilityBonus || [],
      },
    }
    ...
    if (isEditing.value && editingCharacterId.value) {
      ...
      if (result.success) {
        ...
        // Show toast instead of alert when save limit or other error occurs
        const { useNotificationStore } = await import('@/stores/notificationStore')
        useNotificationStore().addNotification({ type: 'error', message: result.error || 'Failed to save character.' })
        throw new Error(result.error)
      }
    }
  } catch (error) {
    ...
    const { useNotificationStore } = await import('@/stores/notificationStore')
    useNotificationStore().addNotification({ type: 'error', message: 'Error saving character: ' + (error?.message || 'Unknown error') })
  }
}
