  ...
  <v-dialog v-model="showAccessDialog" max-width="500">
    <v-card class="access-control-card">
      ...
      <v-card-text class="pa-6">
        <v-alert class="mb-4" color="info" icon="mdi-account-group" variant="tonal">
          <v-alert-title>Private D&D Group</v-alert-title>
          This character creator is private and intended for a specific D&D group. Please enter the access code provided by your Dungeon Master to continue.
        </v-alert>
        ...
        <form id="access-form" action="data:text/plain,success" method="post" @submit="handleFormSubmit">
          ...
          <input aria-hidden="true" autocomplete="username" name="username" style="display: none" type="text" value="dnd-group-member" />
        ...
        <v-alert class="mb-4" density="compact" type="warning" variant="tonal">
          <div class="text-caption">
            ...
            This app is for invited players only. Contact the DM if you should have access.
          </div>
        </v-alert>
      </v-card-text>

      <v-card-actions class="pa-6 pt-0">
        <v-btn class="flex-grow-1" color="primary" :disabled="!accessPasswordInput.trim()" form="access-form" prepend-icon="mdi-check" type="submit" variant="elevated">Enter</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
